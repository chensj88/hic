...
FROM CIS_HLHT..HLHT_MJZBL_MJZBL A,CISDB.dbo.OUTP_ORDER C WHERE A.jzlsh=C.GHXH and A.yzbzxx ='NA';
--医嘱开立科室代码、名称
UPDATE A SET A.yzklksdm =C.KSDM,A.yzklks=C.KSMC
FROM CIS_HLHT..HLHT_MJZBL_MJZBL A,CISDB.dbo.OUTP_ORDER C 
...
--门诊留观
 ...
 --医嘱开立日期时间
  ...
  --专业技术职务类别
  UPDATE A SET A.zyjszwlb = ISNULL(B.ZCDM,'NA'),A.zyjszwlbmc = ISNULL(B.ZCMC,'NA') FROM CIS_HLHT.dbo.HLHT_MJZBL_JZLGBL A LEFT JOIN CISDB.dbo.SYS_ZGDMK B ON B.ID = A.ysbm WHERE (A.zyjszwlb = 'NA' OR A.zyjszwlbmc ='NA')
  --用户去向
