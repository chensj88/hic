CREATE PROCEDURE [dbo].[USP_HLHT_MJZCF_XYCF_DATA]
  ...
  [时间]2018-09-23
  [功能]导出互联互通西药处方信息日数据 ---USP_HLHT_MJZCF_XYCF_DATA
  [参数]
   @sourceType: 元数据类型
   @startime: 开始时间戳
   @endtime:  结束时间戳
  ...
  [调用实例]
  [调用]:
        exec USP_HLHT_MJZCF_XYCF_DATA '1','2018-01-01','2018-01-03','1' --通过首页序号提取数据
        exec USP_HLHT_MJZCF_XYCF_DATA '1','2018010100:00:00','2018010323:59:59',NULL --提取当天的数据
  [注意事项]
    1、在CIS_HLHT中创建
    ...
    begin
     --创建临时表
      ...
      SELECT
          ...
          GETDATE()                                                                                                      AS gxsj
          FROM #OUTP_ORDER_TEMP T1
          INNER JOIN [HLHT_MZ_CIS].[CISDB].[dbo].[OUTP_ORDERITEM] T2(nolock) ON T1.XH = T2.CFXH
          INNER JOIN [HLHT_MZ_CIS].[CISDB].[dbo].[OUTP_JZJLK] T3(nolock) ON T1.GHXH = T3.GHXH
