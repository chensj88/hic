package com.winning.hic.model;


import org.apache.ibatis.type.Alias;

import java.io.Serializable;
import java.sql.Timestamp;


/**
 * @author HLHT
 * @title 
 * @email Winning Health
 * @package com.winning.hic.model
 * @Timestamp 2018-31-31 16:31:46
 */
@Alias("hlhtZlczjlSxjl")
public class HlhtZlczjlSxjl extends BaseDomain implements Serializable {

    private static final long serialVersionUID = -1L;

    /**
     * 字段名：yjlxh
     * 备注: 
     * 默认值：无
     */
    private String yjlxh;
    /**
     * 字段名：jzlsh
     * 备注: 
     * 默认值：无
     */
    private String jzlsh;
    /**
     * 字段名：patid
     * 备注: 
     * 默认值：无
     */
    private String patid;
    /**
     * 字段名：hzxm
     * 备注: 
     * 默认值：无
     */
    private String hzxm;
    /**
     * 字段名：mjzh
     * 备注: 
     * 默认值：无
     */
    private String mjzh;
    /**
     * 字段名：zyh
     * 备注: 
     * 默认值：无
     */
    private String zyh;
    /**
     * 字段名：jzlb
     * 备注: 
     * 默认值：无
     */
    private String jzlb;
    /**
     * 字段名：sfzhm
     * 备注: 
     * 默认值：无
     */
    private String sfzhm;
    /**
     * 字段名：xbdm
     * 备注: 
     * 默认值：无
     */
    private String xbdm;
    /**
     * 字段名：xbmc
     * 备注: 
     * 默认值：无
     */
    private String xbmc;
    /**
     * 字段名：nls
     * 备注: 
     * 默认值：无
     */
    private Short nls;
    /**
     * 字段名：nly
     * 备注: 
     * 默认值：无
     */
    private String nly;
    /**
     * 字段名：dzsqdbh
     * 备注: 
     * 默认值：无
     */
    private String dzsqdbh;
    /**
     * 字段名：ksdm
     * 备注: 
     * 默认值：无
     */
    private String ksdm;
    /**
     * 字段名：ksmc
     * 备注: 
     * 默认值：无
     */
    private String ksmc;
    /**
     * 字段名：bqdm
     * 备注: 
     * 默认值：无
     */
    private String bqdm;
    /**
     * 字段名：bqmc
     * 备注: 
     * 默认值：无
     */
    private String bqmc;
    /**
     * 字段名：bfh
     * 备注: 
     * 默认值：无
     */
    private String bfh;
    /**
     * 字段名：bch
     * 备注: 
     * 默认值：无
     */
    private String bch;
    /**
     * 字段名：aboxx
     * 备注: 
     * 默认值：无
     */
    private String aboxx;
    /**
     * 字段名：aboxxmc
     * 备注: 
     * 默认值：无
     */
    private String aboxxmc;
    /**
     * 字段名：rhxx
     * 备注: 
     * 默认值：无
     */
    private String rhxx;
    /**
     * 字段名：rhxxmc
     * 备注: 
     * 默认值：无
     */
    private String rhxxmc;
    /**
     * 字段名：sxsbz
     * 备注: 
     * 默认值：无
     */
    private String sxsbz;
    /**
     * 字段名：sxsbzmc
     * 备注: 
     * 默认值：无
     */
    private String sxsbzmc;
    /**
     * 字段名：jbzdbm
     * 备注: 
     * 默认值：无
     */
    private String jbzdbm;
    /**
     * 字段名：jbzd
     * 备注: 
     * 默认值：无
     */
    private String jbzd;
    /**
     * 字段名：sxxzdm
     * 备注: 
     * 默认值：无
     */
    private String sxxzdm;
    /**
     * 字段名：sxxzmc
     * 备注: 
     * 默认值：无
     */
    private String sxxzmc;
    /**
     * 字段名：sqaboxx
     * 备注: 
     * 默认值：无
     */
    private String sqaboxx;
    /**
     * 字段名：sqaboxxmc
     * 备注: 
     * 默认值：无
     */
    private String sqaboxxmc;
    /**
     * 字段名：sqrhxx
     * 备注: 
     * 默认值：无
     */
    private String sqrhxx;
    /**
     * 字段名：sqrhxxmc
     * 备注: 
     * 默认值：无
     */
    private String sqrhxxmc;
    /**
     * 字段名：sxzz
     * 备注: 
     * 默认值：无
     */
    private String sxzz;
    /**
     * 字段名：sxgcjl
     * 备注: 
     * 默认值：无
     */
    private String sxgcjl;
    /**
     * 字段名：sxpzdm
     * 备注: 
     * 默认值：无
     */
    private String sxpzdm;
    /**
     * 字段名：sxpzmc
     * 备注: 
     * 默认值：无
     */
    private String sxpzmc;
    /**
     * 字段名：xdbm
     * 备注: 
     * 默认值：无
     */
    private String xdbm;
    /**
     * 字段名：sxl
     * 备注: 
     * 默认值：无
     */
    private Short sxl;
    /**
     * 字段名：sxljldw
     * 备注: 
     * 默认值：无
     */
    private String sxljldw;
    /**
     * 字段名：sxfybz
     * 备注: 
     * 默认值：无
     */
    private String sxfybz;
    /**
     * 字段名：sxfylx
     * 备注: 
     * 默认值：无
     */
    private String sxfylx;
    /**
     * 字段名：sxfylxmc
     * 备注: 
     * 默认值：无
     */
    private String sxfylxmc;
    /**
     * 字段名：sxcs
     * 备注: 
     * 默认值：无
     */
    private Integer sxcs;
    /**
     * 字段名：sxrqsj
     * 备注: 
     * 默认值：无
     */
    private Timestamp sxrqsj;
    /**
     * 字段名：sxyy
     * 备注: 
     * 默认值：无
     */
    private String sxyy;
    /**
     * 字段名：ysbm
     * 备注: 
     * 默认值：无
     */
    private String ysbm;
    /**
     * 字段名：ysqm
     * 备注: 
     * 默认值：无
     */
    private String ysqm;
    /**
     * 字段名：qmrqsj
     * 备注: 
     * 默认值：无
     */
    private Timestamp qmrqsj;
    /**
     * 字段名：gxsj
     * 备注: 
     * 默认值：无
     */
    private Timestamp gxsj;

    public HlhtZlczjlSxjl (){

    }

   /**
   * 字段名：yjlxh
   * get方法
   * 备注: 
   */
   public String getYjlxh(){

        return yjlxh;
   }

   /**
   * 字段名：yjlxh
   * set方法
   * 备注: 
   */
   public void setYjlxh(String yjlxh){
        this.yjlxh = yjlxh;
   }
   /**
   * 字段名：jzlsh
   * get方法
   * 备注: 
   */
   public String getJzlsh(){

        return jzlsh;
   }

   /**
   * 字段名：jzlsh
   * set方法
   * 备注: 
   */
   public void setJzlsh(String jzlsh){
        this.jzlsh = jzlsh;
   }
   /**
   * 字段名：patid
   * get方法
   * 备注: 
   */
   public String getPatid(){

        return patid;
   }

   /**
   * 字段名：patid
   * set方法
   * 备注: 
   */
   public void setPatid(String patid){
        this.patid = patid;
   }
   /**
   * 字段名：hzxm
   * get方法
   * 备注: 
   */
   public String getHzxm(){

        return hzxm;
   }

   /**
   * 字段名：hzxm
   * set方法
   * 备注: 
   */
   public void setHzxm(String hzxm){
        this.hzxm = hzxm;
   }
   /**
   * 字段名：mjzh
   * get方法
   * 备注: 
   */
   public String getMjzh(){

        return mjzh;
   }

   /**
   * 字段名：mjzh
   * set方法
   * 备注: 
   */
   public void setMjzh(String mjzh){
        this.mjzh = mjzh;
   }
   /**
   * 字段名：zyh
   * get方法
   * 备注: 
   */
   public String getZyh(){

        return zyh;
   }

   /**
   * 字段名：zyh
   * set方法
   * 备注: 
   */
   public void setZyh(String zyh){
        this.zyh = zyh;
   }
   /**
   * 字段名：jzlb
   * get方法
   * 备注: 
   */
   public String getJzlb(){

        return jzlb;
   }

   /**
   * 字段名：jzlb
   * set方法
   * 备注: 
   */
   public void setJzlb(String jzlb){
        this.jzlb = jzlb;
   }
   /**
   * 字段名：sfzhm
   * get方法
   * 备注: 
   */
   public String getSfzhm(){

        return sfzhm;
   }

   /**
   * 字段名：sfzhm
   * set方法
   * 备注: 
   */
   public void setSfzhm(String sfzhm){
        this.sfzhm = sfzhm;
   }
   /**
   * 字段名：xbdm
   * get方法
   * 备注: 
   */
   public String getXbdm(){

        return xbdm;
   }

   /**
   * 字段名：xbdm
   * set方法
   * 备注: 
   */
   public void setXbdm(String xbdm){
        this.xbdm = xbdm;
   }
   /**
   * 字段名：xbmc
   * get方法
   * 备注: 
   */
   public String getXbmc(){

        return xbmc;
   }

   /**
   * 字段名：xbmc
   * set方法
   * 备注: 
   */
   public void setXbmc(String xbmc){
        this.xbmc = xbmc;
   }
   /**
   * 字段名：nls
   * get方法
   * 备注: 
   */
   public Short getNls(){

        return nls;
   }

   /**
   * 字段名：nls
   * set方法
   * 备注: 
   */
   public void setNls(Short nls){
        this.nls = nls;
   }
   /**
   * 字段名：nly
   * get方法
   * 备注: 
   */
   public String getNly(){

        return nly;
   }

   /**
   * 字段名：nly
   * set方法
   * 备注: 
   */
   public void setNly(String nly){
        this.nly = nly;
   }
   /**
   * 字段名：dzsqdbh
   * get方法
   * 备注: 
   */
   public String getDzsqdbh(){

        return dzsqdbh;
   }

   /**
   * 字段名：dzsqdbh
   * set方法
   * 备注: 
   */
   public void setDzsqdbh(String dzsqdbh){
        this.dzsqdbh = dzsqdbh;
   }
   /**
   * 字段名：ksdm
   * get方法
   * 备注: 
   */
   public String getKsdm(){

        return ksdm;
   }

   /**
   * 字段名：ksdm
   * set方法
   * 备注: 
   */
   public void setKsdm(String ksdm){
        this.ksdm = ksdm;
   }
   /**
   * 字段名：ksmc
   * get方法
   * 备注: 
   */
   public String getKsmc(){

        return ksmc;
   }

   /**
   * 字段名：ksmc
   * set方法
   * 备注: 
   */
   public void setKsmc(String ksmc){
        this.ksmc = ksmc;
   }
   /**
   * 字段名：bqdm
   * get方法
   * 备注: 
   */
   public String getBqdm(){

        return bqdm;
   }

   /**
   * 字段名：bqdm
   * set方法
   * 备注: 
   */
   public void setBqdm(String bqdm){
        this.bqdm = bqdm;
   }
   /**
   * 字段名：bqmc
   * get方法
   * 备注: 
   */
   public String getBqmc(){

        return bqmc;
   }

   /**
   * 字段名：bqmc
   * set方法
   * 备注: 
   */
   public void setBqmc(String bqmc){
        this.bqmc = bqmc;
   }
   /**
   * 字段名：bfh
   * get方法
   * 备注: 
   */
   public String getBfh(){

        return bfh;
   }

   /**
   * 字段名：bfh
   * set方法
   * 备注: 
   */
   public void setBfh(String bfh){
        this.bfh = bfh;
   }
   /**
   * 字段名：bch
   * get方法
   * 备注: 
   */
   public String getBch(){

        return bch;
   }

   /**
   * 字段名：bch
   * set方法
   * 备注: 
   */
   public void setBch(String bch){
        this.bch = bch;
   }
   /**
   * 字段名：aboxx
   * get方法
   * 备注: 
   */
   public String getAboxx(){

        return aboxx;
   }

   /**
   * 字段名：aboxx
   * set方法
   * 备注: 
   */
   public void setAboxx(String aboxx){
        this.aboxx = aboxx;
   }
   /**
   * 字段名：aboxxmc
   * get方法
   * 备注: 
   */
   public String getAboxxmc(){

        return aboxxmc;
   }

   /**
   * 字段名：aboxxmc
   * set方法
   * 备注: 
   */
   public void setAboxxmc(String aboxxmc){
        this.aboxxmc = aboxxmc;
   }
   /**
   * 字段名：rhxx
   * get方法
   * 备注: 
   */
   public String getRhxx(){

        return rhxx;
   }

   /**
   * 字段名：rhxx
   * set方法
   * 备注: 
   */
   public void setRhxx(String rhxx){
        this.rhxx = rhxx;
   }
   /**
   * 字段名：rhxxmc
   * get方法
   * 备注: 
   */
   public String getRhxxmc(){

        return rhxxmc;
   }

   /**
   * 字段名：rhxxmc
   * set方法
   * 备注: 
   */
   public void setRhxxmc(String rhxxmc){
        this.rhxxmc = rhxxmc;
   }
   /**
   * 字段名：sxsbz
   * get方法
   * 备注: 
   */
   public String getSxsbz(){

        return sxsbz;
   }

   /**
   * 字段名：sxsbz
   * set方法
   * 备注: 
   */
   public void setSxsbz(String sxsbz){
        this.sxsbz = sxsbz;
   }
   /**
   * 字段名：sxsbzmc
   * get方法
   * 备注: 
   */
   public String getSxsbzmc(){

        return sxsbzmc;
   }

   /**
   * 字段名：sxsbzmc
   * set方法
   * 备注: 
   */
   public void setSxsbzmc(String sxsbzmc){
        this.sxsbzmc = sxsbzmc;
   }
   /**
   * 字段名：jbzdbm
   * get方法
   * 备注: 
   */
   public String getJbzdbm(){

        return jbzdbm;
   }

   /**
   * 字段名：jbzdbm
   * set方法
   * 备注: 
   */
   public void setJbzdbm(String jbzdbm){
        this.jbzdbm = jbzdbm;
   }
   /**
   * 字段名：jbzd
   * get方法
   * 备注: 
   */
   public String getJbzd(){

        return jbzd;
   }

   /**
   * 字段名：jbzd
   * set方法
   * 备注: 
   */
   public void setJbzd(String jbzd){
        this.jbzd = jbzd;
   }
   /**
   * 字段名：sxxzdm
   * get方法
   * 备注: 
   */
   public String getSxxzdm(){

        return sxxzdm;
   }

   /**
   * 字段名：sxxzdm
   * set方法
   * 备注: 
   */
   public void setSxxzdm(String sxxzdm){
        this.sxxzdm = sxxzdm;
   }
   /**
   * 字段名：sxxzmc
   * get方法
   * 备注: 
   */
   public String getSxxzmc(){

        return sxxzmc;
   }

   /**
   * 字段名：sxxzmc
   * set方法
   * 备注: 
   */
   public void setSxxzmc(String sxxzmc){
        this.sxxzmc = sxxzmc;
   }
   /**
   * 字段名：sqaboxx
   * get方法
   * 备注: 
   */
   public String getSqaboxx(){

        return sqaboxx;
   }

   /**
   * 字段名：sqaboxx
   * set方法
   * 备注: 
   */
   public void setSqaboxx(String sqaboxx){
        this.sqaboxx = sqaboxx;
   }
   /**
   * 字段名：sqaboxxmc
   * get方法
   * 备注: 
   */
   public String getSqaboxxmc(){

        return sqaboxxmc;
   }

   /**
   * 字段名：sqaboxxmc
   * set方法
   * 备注: 
   */
   public void setSqaboxxmc(String sqaboxxmc){
        this.sqaboxxmc = sqaboxxmc;
   }
   /**
   * 字段名：sqrhxx
   * get方法
   * 备注: 
   */
   public String getSqrhxx(){

        return sqrhxx;
   }

   /**
   * 字段名：sqrhxx
   * set方法
   * 备注: 
   */
   public void setSqrhxx(String sqrhxx){
        this.sqrhxx = sqrhxx;
   }
   /**
   * 字段名：sqrhxxmc
   * get方法
   * 备注: 
   */
   public String getSqrhxxmc(){

        return sqrhxxmc;
   }

   /**
   * 字段名：sqrhxxmc
   * set方法
   * 备注: 
   */
   public void setSqrhxxmc(String sqrhxxmc){
        this.sqrhxxmc = sqrhxxmc;
   }
   /**
   * 字段名：sxzz
   * get方法
   * 备注: 
   */
   public String getSxzz(){

        return sxzz;
   }

   /**
   * 字段名：sxzz
   * set方法
   * 备注: 
   */
   public void setSxzz(String sxzz){
        this.sxzz = sxzz;
   }
   /**
   * 字段名：sxgcjl
   * get方法
   * 备注: 
   */
   public String getSxgcjl(){

        return sxgcjl;
   }

   /**
   * 字段名：sxgcjl
   * set方法
   * 备注: 
   */
   public void setSxgcjl(String sxgcjl){
        this.sxgcjl = sxgcjl;
   }
   /**
   * 字段名：sxpzdm
   * get方法
   * 备注: 
   */
   public String getSxpzdm(){

        return sxpzdm;
   }

   /**
   * 字段名：sxpzdm
   * set方法
   * 备注: 
   */
   public void setSxpzdm(String sxpzdm){
        this.sxpzdm = sxpzdm;
   }
   /**
   * 字段名：sxpzmc
   * get方法
   * 备注: 
   */
   public String getSxpzmc(){

        return sxpzmc;
   }

   /**
   * 字段名：sxpzmc
   * set方法
   * 备注: 
   */
   public void setSxpzmc(String sxpzmc){
        this.sxpzmc = sxpzmc;
   }
   /**
   * 字段名：xdbm
   * get方法
   * 备注: 
   */
   public String getXdbm(){

        return xdbm;
   }

   /**
   * 字段名：xdbm
   * set方法
   * 备注: 
   */
   public void setXdbm(String xdbm){
        this.xdbm = xdbm;
   }
   /**
   * 字段名：sxl
   * get方法
   * 备注: 
   */
   public Short getSxl(){

        return sxl;
   }

   /**
   * 字段名：sxl
   * set方法
   * 备注: 
   */
   public void setSxl(Short sxl){
        this.sxl = sxl;
   }
   /**
   * 字段名：sxljldw
   * get方法
   * 备注: 
   */
   public String getSxljldw(){

        return sxljldw;
   }

   /**
   * 字段名：sxljldw
   * set方法
   * 备注: 
   */
   public void setSxljldw(String sxljldw){
        this.sxljldw = sxljldw;
   }
   /**
   * 字段名：sxfybz
   * get方法
   * 备注: 
   */
   public String getSxfybz(){

        return sxfybz;
   }

   /**
   * 字段名：sxfybz
   * set方法
   * 备注: 
   */
   public void setSxfybz(String sxfybz){
        this.sxfybz = sxfybz;
   }
   /**
   * 字段名：sxfylx
   * get方法
   * 备注: 
   */
   public String getSxfylx(){

        return sxfylx;
   }

   /**
   * 字段名：sxfylx
   * set方法
   * 备注: 
   */
   public void setSxfylx(String sxfylx){
        this.sxfylx = sxfylx;
   }
   /**
   * 字段名：sxfylxmc
   * get方法
   * 备注: 
   */
   public String getSxfylxmc(){

        return sxfylxmc;
   }

   /**
   * 字段名：sxfylxmc
   * set方法
   * 备注: 
   */
   public void setSxfylxmc(String sxfylxmc){
        this.sxfylxmc = sxfylxmc;
   }
   /**
   * 字段名：sxcs
   * get方法
   * 备注: 
   */
   public Integer getSxcs(){

        return sxcs;
   }

   /**
   * 字段名：sxcs
   * set方法
   * 备注: 
   */
   public void setSxcs(Integer sxcs){
        this.sxcs = sxcs;
   }
   /**
   * 字段名：sxrqsj
   * get方法
   * 备注: 
   */
   public Timestamp getSxrqsj(){

        return sxrqsj;
   }

   /**
   * 字段名：sxrqsj
   * set方法
   * 备注: 
   */
   public void setSxrqsj(Timestamp sxrqsj){
        this.sxrqsj = sxrqsj;
   }
   /**
   * 字段名：sxyy
   * get方法
   * 备注: 
   */
   public String getSxyy(){

        return sxyy;
   }

   /**
   * 字段名：sxyy
   * set方法
   * 备注: 
   */
   public void setSxyy(String sxyy){
        this.sxyy = sxyy;
   }
   /**
   * 字段名：ysbm
   * get方法
   * 备注: 
   */
   public String getYsbm(){

        return ysbm;
   }

   /**
   * 字段名：ysbm
   * set方法
   * 备注: 
   */
   public void setYsbm(String ysbm){
        this.ysbm = ysbm;
   }
   /**
   * 字段名：ysqm
   * get方法
   * 备注: 
   */
   public String getYsqm(){

        return ysqm;
   }

   /**
   * 字段名：ysqm
   * set方法
   * 备注: 
   */
   public void setYsqm(String ysqm){
        this.ysqm = ysqm;
   }
   /**
   * 字段名：qmrqsj
   * get方法
   * 备注: 
   */
   public Timestamp getQmrqsj(){

        return qmrqsj;
   }

   /**
   * 字段名：qmrqsj
   * set方法
   * 备注: 
   */
   public void setQmrqsj(Timestamp qmrqsj){
        this.qmrqsj = qmrqsj;
   }
   /**
   * 字段名：gxsj
   * get方法
   * 备注: 
   */
   public Timestamp getGxsj(){

        return gxsj;
   }

   /**
   * 字段名：gxsj
   * set方法
   * 备注: 
   */
   public void setGxsj(Timestamp gxsj){
        this.gxsj = gxsj;
   }

}