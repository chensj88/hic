package com.winning.hic.model;

import java.math.BigDecimal; 
import java.util.Date; 
import java.util.Date; 
import java.util.Date; 
import java.io.Serializable; 

import org.apache.ibatis.type.Alias; 

import com.winning.hic.model.BaseDomain;



/**
 * @author HLHT
 * @title 
 * @email Winning Health
 * @package com.winning.hic.model
 * @date 2018-31-31 16:31:27
 */
@Alias("hlhtZlczjlMzshfsjl")
public class HlhtZlczjlMzshfsjl extends BaseDomain implements Serializable {

    private static final long serialVersionUID = -1L;

    /**
     * 字段名：yjlxh
     * 备注: 
     * 默认值：无
     */
    private String yjlxh;
    /**
     * 字段名：jzlsh
     * 备注: 
     * 默认值：无
     */
    private String jzlsh;
    /**
     * 字段名：patid
     * 备注: 
     * 默认值：无
     */
    private String patid;
    /**
     * 字段名：hzxm
     * 备注: 
     * 默认值：无
     */
    private String hzxm;
    /**
     * 字段名：mjzh
     * 备注: 
     * 默认值：无
     */
    private String mjzh;
    /**
     * 字段名：zyh
     * 备注: 
     * 默认值：无
     */
    private String zyh;
    /**
     * 字段名：jzlb
     * 备注: 
     * 默认值：无
     */
    private String jzlb;
    /**
     * 字段名：sfzhm
     * 备注: 
     * 默认值：无
     */
    private String sfzhm;
    /**
     * 字段名：xbdm
     * 备注: 
     * 默认值：无
     */
    private String xbdm;
    /**
     * 字段名：xbmc
     * 备注: 
     * 默认值：无
     */
    private String xbmc;
    /**
     * 字段名：nls
     * 备注: 
     * 默认值：无
     */
    private Short nls;
    /**
     * 字段名：nly
     * 备注: 
     * 默认值：无
     */
    private String nly;
    /**
     * 字段名：tz
     * 备注: 
     * 默认值：无
     */
    private BigDecimal tz;
    /**
     * 字段名：dzsqsbh
     * 备注: 
     * 默认值：无
     */
    private String dzsqsbh;
    /**
     * 字段名：zzjgdm
     * 备注: 
     * 默认值：无
     */
    private String zzjgdm;
    /**
     * 字段名：zzjgmc
     * 备注: 
     * 默认值：无
     */
    private String zzjgmc;
    /**
     * 字段名：ksdm
     * 备注: 
     * 默认值：无
     */
    private String ksdm;
    /**
     * 字段名：ksmc
     * 备注: 
     * 默认值：无
     */
    private String ksmc;
    /**
     * 字段名：bqdm
     * 备注: 
     * 默认值：无
     */
    private String bqdm;
    /**
     * 字段名：bqmc
     * 备注: 
     * 默认值：无
     */
    private String bqmc;
    /**
     * 字段名：bfh
     * 备注: 
     * 默认值：无
     */
    private String bfh;
    /**
     * 字段名：bch
     * 备注: 
     * 默认值：无
     */
    private String bch;
    /**
     * 字段名：aboxx
     * 备注: 
     * 默认值：无
     */
    private String aboxx;
    /**
     * 字段名：aboxxmc
     * 备注: 
     * 默认值：无
     */
    private String aboxxmc;
    /**
     * 字段名：rhxx
     * 备注: 
     * 默认值：无
     */
    private String rhxx;
    /**
     * 字段名：rhxxmc
     * 备注: 
     * 默认值：无
     */
    private String rhxxmc;
    /**
     * 字段名：sqzdbm
     * 备注: 
     * 默认值：无
     */
    private String sqzdbm;
    /**
     * 字段名：sqzdmc
     * 备注: 
     * 默认值：无
     */
    private String sqzdmc;
    /**
     * 字段名：shzdbm
     * 备注: 
     * 默认值：无
     */
    private String shzdbm;
    /**
     * 字段名：shzdmc
     * 备注: 
     * 默认值：无
     */
    private String shzdmc;
    /**
     * 字段名：ybzkjcjg
     * 备注: 
     * 默认值：无
     */
    private String ybzkjcjg;
    /**
     * 字段名：ssjczbm
     * 备注: 
     * 默认值：无
     */
    private String ssjczbm;
    /**
     * 字段名：ssjczmc
     * 备注: 
     * 默认值：无
     */
    private String ssjczmc;
    /**
     * 字段名：mzff
     * 备注: 
     * 默认值：无
     */
    private String mzff;
    /**
     * 字段名：mzffmc
     * 备注: 
     * 默认值：无
     */
    private String mzffmc;
    /**
     * 字段名：mzhfqk
     * 备注: 
     * 默认值：无
     */
    private String mzhfqk;
    /**
     * 字段名：qxrqsj
     * 备注: 
     * 默认值：无
     */
    private Date qxrqsj;
    /**
     * 字段名：bcqgcgbz
     * 备注: 
     * 默认值：无
     */
    private String bcqgcgbz;
    /**
     * 字段名：tsqk
     * 备注: 
     * 默认值：无
     */
    private String tsqk;
    /**
     * 字段名：mzsyz
     * 备注: 
     * 默认值：无
     */
    private String mzsyz;
    /**
     * 字段名：mzysbm
     * 备注: 
     * 默认值：无
     */
    private String mzysbm;
    /**
     * 字段名：mzysqm
     * 备注: 
     * 默认值：无
     */
    private String mzysqm;
    /**
     * 字段名：qmrqsj
     * 备注: 
     * 默认值：无
     */
    private Date qmrqsj;
    /**
     * 字段名：gxsj
     * 备注: 
     * 默认值：无
     */
    private Date gxsj;
    /**
     * 字段名：syxh
     * 备注: 
     * 默认值：无
     */
    private String syxh;
    /**
     * 字段名：tjzt
     * 备注: 
     * 默认值：无
     */
    private String tjzt;
    /**
     * 字段名：yxjl
     * 备注: 
     * 默认值：无
     */
    private Integer yxjl;

    public HlhtZlczjlMzshfsjl (){

    }

   /**
   * 字段名：yjlxh
   * get方法
   * 备注: 
   */
   public String getYjlxh(){

        return yjlxh;
   }

   /**
   * 字段名：yjlxh
   * set方法
   * 备注: 
   */
   public void setYjlxh(String yjlxh){
        this.yjlxh = yjlxh;
   }
   /**
   * 字段名：jzlsh
   * get方法
   * 备注: 
   */
   public String getJzlsh(){

        return jzlsh;
   }

   /**
   * 字段名：jzlsh
   * set方法
   * 备注: 
   */
   public void setJzlsh(String jzlsh){
        this.jzlsh = jzlsh;
   }
   /**
   * 字段名：patid
   * get方法
   * 备注: 
   */
   public String getPatid(){

        return patid;
   }

   /**
   * 字段名：patid
   * set方法
   * 备注: 
   */
   public void setPatid(String patid){
        this.patid = patid;
   }
   /**
   * 字段名：hzxm
   * get方法
   * 备注: 
   */
   public String getHzxm(){

        return hzxm;
   }

   /**
   * 字段名：hzxm
   * set方法
   * 备注: 
   */
   public void setHzxm(String hzxm){
        this.hzxm = hzxm;
   }
   /**
   * 字段名：mjzh
   * get方法
   * 备注: 
   */
   public String getMjzh(){

        return mjzh;
   }

   /**
   * 字段名：mjzh
   * set方法
   * 备注: 
   */
   public void setMjzh(String mjzh){
        this.mjzh = mjzh;
   }
   /**
   * 字段名：zyh
   * get方法
   * 备注: 
   */
   public String getZyh(){

        return zyh;
   }

   /**
   * 字段名：zyh
   * set方法
   * 备注: 
   */
   public void setZyh(String zyh){
        this.zyh = zyh;
   }
   /**
   * 字段名：jzlb
   * get方法
   * 备注: 
   */
   public String getJzlb(){

        return jzlb;
   }

   /**
   * 字段名：jzlb
   * set方法
   * 备注: 
   */
   public void setJzlb(String jzlb){
        this.jzlb = jzlb;
   }
   /**
   * 字段名：sfzhm
   * get方法
   * 备注: 
   */
   public String getSfzhm(){

        return sfzhm;
   }

   /**
   * 字段名：sfzhm
   * set方法
   * 备注: 
   */
   public void setSfzhm(String sfzhm){
        this.sfzhm = sfzhm;
   }
   /**
   * 字段名：xbdm
   * get方法
   * 备注: 
   */
   public String getXbdm(){

        return xbdm;
   }

   /**
   * 字段名：xbdm
   * set方法
   * 备注: 
   */
   public void setXbdm(String xbdm){
        this.xbdm = xbdm;
   }
   /**
   * 字段名：xbmc
   * get方法
   * 备注: 
   */
   public String getXbmc(){

        return xbmc;
   }

   /**
   * 字段名：xbmc
   * set方法
   * 备注: 
   */
   public void setXbmc(String xbmc){
        this.xbmc = xbmc;
   }
   /**
   * 字段名：nls
   * get方法
   * 备注: 
   */
   public Short getNls(){

        return nls;
   }

   /**
   * 字段名：nls
   * set方法
   * 备注: 
   */
   public void setNls(Short nls){
        this.nls = nls;
   }
   /**
   * 字段名：nly
   * get方法
   * 备注: 
   */
   public String getNly(){

        return nly;
   }

   /**
   * 字段名：nly
   * set方法
   * 备注: 
   */
   public void setNly(String nly){
        this.nly = nly;
   }
   /**
   * 字段名：tz
   * get方法
   * 备注: 
   */
   public BigDecimal getTz(){

        return tz;
   }

   /**
   * 字段名：tz
   * set方法
   * 备注: 
   */
   public void setTz(BigDecimal tz){
        this.tz = tz;
   }
   /**
   * 字段名：dzsqsbh
   * get方法
   * 备注: 
   */
   public String getDzsqsbh(){

        return dzsqsbh;
   }

   /**
   * 字段名：dzsqsbh
   * set方法
   * 备注: 
   */
   public void setDzsqsbh(String dzsqsbh){
        this.dzsqsbh = dzsqsbh;
   }
   /**
   * 字段名：zzjgdm
   * get方法
   * 备注: 
   */
   public String getZzjgdm(){

        return zzjgdm;
   }

   /**
   * 字段名：zzjgdm
   * set方法
   * 备注: 
   */
   public void setZzjgdm(String zzjgdm){
        this.zzjgdm = zzjgdm;
   }
   /**
   * 字段名：zzjgmc
   * get方法
   * 备注: 
   */
   public String getZzjgmc(){

        return zzjgmc;
   }

   /**
   * 字段名：zzjgmc
   * set方法
   * 备注: 
   */
   public void setZzjgmc(String zzjgmc){
        this.zzjgmc = zzjgmc;
   }
   /**
   * 字段名：ksdm
   * get方法
   * 备注: 
   */
   public String getKsdm(){

        return ksdm;
   }

   /**
   * 字段名：ksdm
   * set方法
   * 备注: 
   */
   public void setKsdm(String ksdm){
        this.ksdm = ksdm;
   }
   /**
   * 字段名：ksmc
   * get方法
   * 备注: 
   */
   public String getKsmc(){

        return ksmc;
   }

   /**
   * 字段名：ksmc
   * set方法
   * 备注: 
   */
   public void setKsmc(String ksmc){
        this.ksmc = ksmc;
   }
   /**
   * 字段名：bqdm
   * get方法
   * 备注: 
   */
   public String getBqdm(){

        return bqdm;
   }

   /**
   * 字段名：bqdm
   * set方法
   * 备注: 
   */
   public void setBqdm(String bqdm){
        this.bqdm = bqdm;
   }
   /**
   * 字段名：bqmc
   * get方法
   * 备注: 
   */
   public String getBqmc(){

        return bqmc;
   }

   /**
   * 字段名：bqmc
   * set方法
   * 备注: 
   */
   public void setBqmc(String bqmc){
        this.bqmc = bqmc;
   }
   /**
   * 字段名：bfh
   * get方法
   * 备注: 
   */
   public String getBfh(){

        return bfh;
   }

   /**
   * 字段名：bfh
   * set方法
   * 备注: 
   */
   public void setBfh(String bfh){
        this.bfh = bfh;
   }
   /**
   * 字段名：bch
   * get方法
   * 备注: 
   */
   public String getBch(){

        return bch;
   }

   /**
   * 字段名：bch
   * set方法
   * 备注: 
   */
   public void setBch(String bch){
        this.bch = bch;
   }
   /**
   * 字段名：aboxx
   * get方法
   * 备注: 
   */
   public String getAboxx(){

        return aboxx;
   }

   /**
   * 字段名：aboxx
   * set方法
   * 备注: 
   */
   public void setAboxx(String aboxx){
        this.aboxx = aboxx;
   }
   /**
   * 字段名：aboxxmc
   * get方法
   * 备注: 
   */
   public String getAboxxmc(){

        return aboxxmc;
   }

   /**
   * 字段名：aboxxmc
   * set方法
   * 备注: 
   */
   public void setAboxxmc(String aboxxmc){
        this.aboxxmc = aboxxmc;
   }
   /**
   * 字段名：rhxx
   * get方法
   * 备注: 
   */
   public String getRhxx(){

        return rhxx;
   }

   /**
   * 字段名：rhxx
   * set方法
   * 备注: 
   */
   public void setRhxx(String rhxx){
        this.rhxx = rhxx;
   }
   /**
   * 字段名：rhxxmc
   * get方法
   * 备注: 
   */
   public String getRhxxmc(){

        return rhxxmc;
   }

   /**
   * 字段名：rhxxmc
   * set方法
   * 备注: 
   */
   public void setRhxxmc(String rhxxmc){
        this.rhxxmc = rhxxmc;
   }
   /**
   * 字段名：sqzdbm
   * get方法
   * 备注: 
   */
   public String getSqzdbm(){

        return sqzdbm;
   }

   /**
   * 字段名：sqzdbm
   * set方法
   * 备注: 
   */
   public void setSqzdbm(String sqzdbm){
        this.sqzdbm = sqzdbm;
   }
   /**
   * 字段名：sqzdmc
   * get方法
   * 备注: 
   */
   public String getSqzdmc(){

        return sqzdmc;
   }

   /**
   * 字段名：sqzdmc
   * set方法
   * 备注: 
   */
   public void setSqzdmc(String sqzdmc){
        this.sqzdmc = sqzdmc;
   }
   /**
   * 字段名：shzdbm
   * get方法
   * 备注: 
   */
   public String getShzdbm(){

        return shzdbm;
   }

   /**
   * 字段名：shzdbm
   * set方法
   * 备注: 
   */
   public void setShzdbm(String shzdbm){
        this.shzdbm = shzdbm;
   }
   /**
   * 字段名：shzdmc
   * get方法
   * 备注: 
   */
   public String getShzdmc(){

        return shzdmc;
   }

   /**
   * 字段名：shzdmc
   * set方法
   * 备注: 
   */
   public void setShzdmc(String shzdmc){
        this.shzdmc = shzdmc;
   }
   /**
   * 字段名：ybzkjcjg
   * get方法
   * 备注: 
   */
   public String getYbzkjcjg(){

        return ybzkjcjg;
   }

   /**
   * 字段名：ybzkjcjg
   * set方法
   * 备注: 
   */
   public void setYbzkjcjg(String ybzkjcjg){
        this.ybzkjcjg = ybzkjcjg;
   }
   /**
   * 字段名：ssjczbm
   * get方法
   * 备注: 
   */
   public String getSsjczbm(){

        return ssjczbm;
   }

   /**
   * 字段名：ssjczbm
   * set方法
   * 备注: 
   */
   public void setSsjczbm(String ssjczbm){
        this.ssjczbm = ssjczbm;
   }
   /**
   * 字段名：ssjczmc
   * get方法
   * 备注: 
   */
   public String getSsjczmc(){

        return ssjczmc;
   }

   /**
   * 字段名：ssjczmc
   * set方法
   * 备注: 
   */
   public void setSsjczmc(String ssjczmc){
        this.ssjczmc = ssjczmc;
   }
   /**
   * 字段名：mzff
   * get方法
   * 备注: 
   */
   public String getMzff(){

        return mzff;
   }

   /**
   * 字段名：mzff
   * set方法
   * 备注: 
   */
   public void setMzff(String mzff){
        this.mzff = mzff;
   }
   /**
   * 字段名：mzffmc
   * get方法
   * 备注: 
   */
   public String getMzffmc(){

        return mzffmc;
   }

   /**
   * 字段名：mzffmc
   * set方法
   * 备注: 
   */
   public void setMzffmc(String mzffmc){
        this.mzffmc = mzffmc;
   }
   /**
   * 字段名：mzhfqk
   * get方法
   * 备注: 
   */
   public String getMzhfqk(){

        return mzhfqk;
   }

   /**
   * 字段名：mzhfqk
   * set方法
   * 备注: 
   */
   public void setMzhfqk(String mzhfqk){
        this.mzhfqk = mzhfqk;
   }
   /**
   * 字段名：qxrqsj
   * get方法
   * 备注: 
   */
   public Date getQxrqsj(){

        return qxrqsj;
   }

   /**
   * 字段名：qxrqsj
   * set方法
   * 备注: 
   */
   public void setQxrqsj(Date qxrqsj){
        this.qxrqsj = qxrqsj;
   }
   /**
   * 字段名：bcqgcgbz
   * get方法
   * 备注: 
   */
   public String getBcqgcgbz(){

        return bcqgcgbz;
   }

   /**
   * 字段名：bcqgcgbz
   * set方法
   * 备注: 
   */
   public void setBcqgcgbz(String bcqgcgbz){
        this.bcqgcgbz = bcqgcgbz;
   }
   /**
   * 字段名：tsqk
   * get方法
   * 备注: 
   */
   public String getTsqk(){

        return tsqk;
   }

   /**
   * 字段名：tsqk
   * set方法
   * 备注: 
   */
   public void setTsqk(String tsqk){
        this.tsqk = tsqk;
   }
   /**
   * 字段名：mzsyz
   * get方法
   * 备注: 
   */
   public String getMzsyz(){

        return mzsyz;
   }

   /**
   * 字段名：mzsyz
   * set方法
   * 备注: 
   */
   public void setMzsyz(String mzsyz){
        this.mzsyz = mzsyz;
   }
   /**
   * 字段名：mzysbm
   * get方法
   * 备注: 
   */
   public String getMzysbm(){

        return mzysbm;
   }

   /**
   * 字段名：mzysbm
   * set方法
   * 备注: 
   */
   public void setMzysbm(String mzysbm){
        this.mzysbm = mzysbm;
   }
   /**
   * 字段名：mzysqm
   * get方法
   * 备注: 
   */
   public String getMzysqm(){

        return mzysqm;
   }

   /**
   * 字段名：mzysqm
   * set方法
   * 备注: 
   */
   public void setMzysqm(String mzysqm){
        this.mzysqm = mzysqm;
   }
   /**
   * 字段名：qmrqsj
   * get方法
   * 备注: 
   */
   public Date getQmrqsj(){

        return qmrqsj;
   }

   /**
   * 字段名：qmrqsj
   * set方法
   * 备注: 
   */
   public void setQmrqsj(Date qmrqsj){
        this.qmrqsj = qmrqsj;
   }
   /**
   * 字段名：gxsj
   * get方法
   * 备注: 
   */
   public Date getGxsj(){

        return gxsj;
   }

   /**
   * 字段名：gxsj
   * set方法
   * 备注: 
   */
   public void setGxsj(Date gxsj){
        this.gxsj = gxsj;
   }
   /**
   * 字段名：syxh
   * get方法
   * 备注: 
   */
   public String getSyxh(){

        return syxh;
   }

   /**
   * 字段名：syxh
   * set方法
   * 备注: 
   */
   public void setSyxh(String syxh){
        this.syxh = syxh;
   }
   /**
   * 字段名：tjzt
   * get方法
   * 备注: 
   */
   public String getTjzt(){

        return tjzt;
   }

   /**
   * 字段名：tjzt
   * set方法
   * 备注: 
   */
   public void setTjzt(String tjzt){
        this.tjzt = tjzt;
   }
   /**
   * 字段名：yxjl
   * get方法
   * 备注: 
   */
   public Integer getYxjl(){

        return yxjl;
   }

   /**
   * 字段名：yxjl
   * set方法
   * 备注: 
   */
   public void setYxjl(Integer yxjl){
        this.yxjl = yxjl;
   }

}