package com.winning.hic.model;

import java.io.Serializable;
import org.apache.ibatis.type.Alias;

/**
 * Coder AutoGenerator generate.
 *
 * @author Coder AutoGenerator
 * @date 2018-07-30 13:02:58
 */
 
@Alias("emrQtbljlk")
public class EmrQtbljlk extends BaseDomain implements Serializable {

	private static final long serialVersionUID = -1L;

	private Long qtbljlxh;
	
	private Integer syxh;
	
	private String bldm;
	
	private String blmc;
	
	private String blxsmc;
	
	private String blms;
	
	private String blnr;
	
	private String mxfldm;
	
	private String cjys;
	
	private String tjys;
	
	private String shys;
	
	private String cjsj;
	
	private String tjsj;
	
	private String shsj;
	
	private Integer yxjl;
	
	private Integer tjzt;
	
	private Integer dyzt;
	
	private Integer dzqmbz;
	
	private String containerid;
	
	private String fssj;
	
	private Integer instanceversion;
	
	private Integer dczt;
	
	private String dcczsj;
	
	private String gxrq;
	
	private String yshysdm;
	
	private String yshysxm;
	
	private String cjksdm;
	
	private String cjksmc;
	
	public EmrQtbljlk() {

	}

	public Long getQtbljlxh() {
		return qtbljlxh;
	}
	
	public void setQtbljlxh(Long qtbljlxh) {
		this.qtbljlxh = qtbljlxh;
	}
	
	public Integer getSyxh() {
		return syxh;
	}
	
	public void setSyxh(Integer syxh) {
		this.syxh = syxh;
	}
	
	public String getBldm() {
		return bldm;
	}
	
	public void setBldm(String bldm) {
		this.bldm = bldm;
	}
	
	public String getBlmc() {
		return blmc;
	}
	
	public void setBlmc(String blmc) {
		this.blmc = blmc;
	}
	
	public String getBlxsmc() {
		return blxsmc;
	}
	
	public void setBlxsmc(String blxsmc) {
		this.blxsmc = blxsmc;
	}
	
	public String getBlms() {
		return blms;
	}
	
	public void setBlms(String blms) {
		this.blms = blms;
	}
	
	public String getBlnr() {
		return blnr;
	}
	
	public void setBlnr(String blnr) {
		this.blnr = blnr;
	}
	
	public String getMxfldm() {
		return mxfldm;
	}
	
	public void setMxfldm(String mxfldm) {
		this.mxfldm = mxfldm;
	}
	
	public String getCjys() {
		return cjys;
	}
	
	public void setCjys(String cjys) {
		this.cjys = cjys;
	}
	
	public String getTjys() {
		return tjys;
	}
	
	public void setTjys(String tjys) {
		this.tjys = tjys;
	}
	
	public String getShys() {
		return shys;
	}
	
	public void setShys(String shys) {
		this.shys = shys;
	}
	
	public String getCjsj() {
		return cjsj;
	}
	
	public void setCjsj(String cjsj) {
		this.cjsj = cjsj;
	}
	
	public String getTjsj() {
		return tjsj;
	}
	
	public void setTjsj(String tjsj) {
		this.tjsj = tjsj;
	}
	
	public String getShsj() {
		return shsj;
	}
	
	public void setShsj(String shsj) {
		this.shsj = shsj;
	}
	
	public Integer getYxjl() {
		return yxjl;
	}
	
	public void setYxjl(Integer yxjl) {
		this.yxjl = yxjl;
	}
	
	public Integer getTjzt() {
		return tjzt;
	}
	
	public void setTjzt(Integer tjzt) {
		this.tjzt = tjzt;
	}
	
	public Integer getDyzt() {
		return dyzt;
	}
	
	public void setDyzt(Integer dyzt) {
		this.dyzt = dyzt;
	}
	
	public Integer getDzqmbz() {
		return dzqmbz;
	}
	
	public void setDzqmbz(Integer dzqmbz) {
		this.dzqmbz = dzqmbz;
	}
	
	public String getContainerid() {
		return containerid;
	}
	
	public void setContainerid(String containerid) {
		this.containerid = containerid;
	}
	
	public String getFssj() {
		return fssj;
	}
	
	public void setFssj(String fssj) {
		this.fssj = fssj;
	}
	
	public Integer getInstanceversion() {
		return instanceversion;
	}
	
	public void setInstanceversion(Integer instanceversion) {
		this.instanceversion = instanceversion;
	}
	
	public Integer getDczt() {
		return dczt;
	}
	
	public void setDczt(Integer dczt) {
		this.dczt = dczt;
	}
	
	public String getDcczsj() {
		return dcczsj;
	}
	
	public void setDcczsj(String dcczsj) {
		this.dcczsj = dcczsj;
	}
	
	public String getGxrq() {
		return gxrq;
	}
	
	public void setGxrq(String gxrq) {
		this.gxrq = gxrq;
	}
	
	public String getYshysdm() {
		return yshysdm;
	}
	
	public void setYshysdm(String yshysdm) {
		this.yshysdm = yshysdm;
	}
	
	public String getYshysxm() {
		return yshysxm;
	}
	
	public void setYshysxm(String yshysxm) {
		this.yshysxm = yshysxm;
	}
	
	public String getCjksdm() {
		return cjksdm;
	}
	
	public void setCjksdm(String cjksdm) {
		this.cjksdm = cjksdm;
	}
	
	public String getCjksmc() {
		return cjksmc;
	}
	
	public void setCjksmc(String cjksmc) {
		this.cjksmc = cjksmc;
	}
	
}