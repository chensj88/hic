package com.winning.hic.model;

import java.sql.Timestamp;
import java.io.Serializable;
import java.sql.Date;

import org.apache.ibatis.type.Alias; 

import com.winning.hic.model.BaseDomain;



/**
 * @author HLHT
 * @title 
 * @email Winning Health
 * @package com.winning.hic.model
 * @date 2018-29-31 16:29:44
 */
@Alias("hlhtMjzblMjzbl")
public class HlhtMjzblMjzbl extends BaseDomain implements Serializable {

    private static final long serialVersionUID = -1L;

    /**
     * 字段名：yjlxh
     * 备注: 
     * 默认值：无
     */
    private String yjlxh;
    /**
     * 字段名：jzlsh
     * 备注: 
     * 默认值：无
     */
    private String jzlsh;
    /**
     * 字段名：patid
     * 备注: 
     * 默认值：无
     */
    private String patid;
    /**
     * 字段名：hzxm
     * 备注: 
     * 默认值：无
     */
    private String hzxm;
    /**
     * 字段名：mjzh
     * 备注: 
     * 默认值：无
     */
    private String mjzh;
    /**
     * 字段名：zyh
     * 备注: 
     * 默认值：无
     */
    private String zyh;
    /**
     * 字段名：sfzhm
     * 备注: 
     * 默认值：无
     */
    private String sfzhm;
    /**
     * 字段名：xbdm
     * 备注: 
     * 默认值：无
     */
    private String xbdm;
    /**
     * 字段名：xbmc
     * 备注: 
     * 默认值：无
     */
    private String xbmc;
    /**
     * 字段名：csrq
     * 备注: 
     * 默认值：无
     */
    private Date csrq;
    /**
     * 字段名：nls
     * 备注: 
     * 默认值：无
     */
    private Short nls;
    /**
     * 字段名：nly
     * 备注: 
     * 默认值：无
     */
    private String nly;
    /**
     * 字段名：zzjgdm
     * 备注: 
     * 默认值：无
     */
    private String zzjgdm;
    /**
     * 字段名：zzjgmc
     * 备注: 
     * 默认值：无
     */
    private String zzjgmc;
    /**
     * 字段名：ksdm
     * 备注: 
     * 默认值：无
     */
    private String ksdm;
    /**
     * 字段名：ksmc
     * 备注: 
     * 默认值：无
     */
    private String ksmc;
    /**
     * 字段名：gmsbz
     * 备注: 
     * 默认值：无
     */
    private String gmsbz;
    /**
     * 字段名：gms
     * 备注: 
     * 默认值：无
     */
    private String gms;
    /**
     * 字段名：jzrqsj
     * 备注: 
     * 默认值：无
     */
    private Timestamp jzrqsj;
    /**
     * 字段名：czbzdm
     * 备注: 
     * 默认值：无
     */
    private String czbzdm;
    /**
     * 字段名：czbzmc
     * 备注: 
     * 默认值：无
     */
    private String czbzmc;
    /**
     * 字段名：zs
     * 备注: 
     * 默认值：无
     */
    private String zs;
    /**
     * 字段名：xbs
     * 备注: 
     * 默认值：无
     */
    private String xbs;
    /**
     * 字段名：jws
     * 备注: 
     * 默认值：无
     */
    private String jws;
    /**
     * 字段名：tgjc
     * 备注: 
     * 默认值：无
     */
    private String tgjc;
    /**
     * 字段名：zyszgcjg
     * 备注: 
     * 默认值：无
     */
    private String zyszgcjg;
    /**
     * 字段名：fzjcxm
     * 备注: 
     * 默认值：无
     */
    private String fzjcxm;
    /**
     * 字段名：fzjcjg
     * 备注: 
     * 默认值：无
     */
    private String fzjcjg;
    /**
     * 字段名：xyzdbm
     * 备注: 
     * 默认值：无
     */
    private String xyzdbm;
    /**
     * 字段名：xyzdmc
     * 备注: 
     * 默认值：无
     */
    private String xyzdmc;
    /**
     * 字段名：zybmdm
     * 备注: 
     * 默认值：无
     */
    private String zybmdm;
    /**
     * 字段名：zybmmc
     * 备注: 
     * 默认值：无
     */
    private String zybmmc;
    /**
     * 字段名：zyzhdm
     * 备注: 
     * 默认值：无
     */
    private String zyzhdm;
    /**
     * 字段名：zyzhmc
     * 备注: 
     * 默认值：无
     */
    private String zyzhmc;
    /**
     * 字段名：bzyj
     * 备注: 
     * 默认值：无
     */
    private String bzyj;
    /**
     * 字段名：zfbm
     * 备注: 
     * 默认值：无
     */
    private String zfbm;
    /**
     * 字段名：zzzf
     * 备注: 
     * 默认值：无
     */
    private String zzzf;
    /**
     * 字段名：yzxmlx
     * 备注: 
     * 默认值：无
     */
    private String yzxmlx;
    /**
     * 字段名：yzxmlxmc
     * 备注: 
     * 默认值：无
     */
    private String yzxmlxmc;
    /**
     * 字段名：yzxmnr
     * 备注: 
     * 默认值：无
     */
    private String yzxmnr;
    /**
     * 字段名：yzbzxx
     * 备注: 
     * 默认值：无
     */
    private String yzbzxx;
    /**
     * 字段名：yzklksdm
     * 备注: 
     * 默认值：无
     */
    private String yzklksdm;
    /**
     * 字段名：yzklks
     * 备注: 
     * 默认值：无
     */
    private String yzklks;
    /**
     * 字段名：yzklzdm
     * 备注: 
     * 默认值：无
     */
    private String yzklzdm;
    /**
     * 字段名：yzklzqm
     * 备注: 
     * 默认值：无
     */
    private String yzklzqm;
    /**
     * 字段名：yzklrq
     * 备注: 
     * 默认值：无
     */
    private Timestamp yzklrq;
    /**
     * 字段名：yzshzdm
     * 备注: 
     * 默认值：无
     */
    private String yzshzdm;
    /**
     * 字段名：yzshzqm
     * 备注: 
     * 默认值：无
     */
    private String yzshzqm;
    /**
     * 字段名：yzshrq
     * 备注: 
     * 默认值：无
     */
    private Timestamp yzshrq;
    /**
     * 字段名：zxksdm
     * 备注: 
     * 默认值：无
     */
    private String zxksdm;
    /**
     * 字段名：yzzxks
     * 备注: 
     * 默认值：无
     */
    private String yzzxks;
    /**
     * 字段名：yzzxzdm
     * 备注: 
     * 默认值：无
     */
    private String yzzxzdm;
    /**
     * 字段名：yzzxzqm
     * 备注: 
     * 默认值：无
     */
    private String yzzxzqm;
    /**
     * 字段名：yzzxrq
     * 备注: 
     * 默认值：无
     */
    private Timestamp yzzxrq;
    /**
     * 字段名：yzzxzt
     * 备注: 
     * 默认值：无
     */
    private String yzzxzt;
    /**
     * 字段名：qxyzzdm
     * 备注: 
     * 默认值：无
     */
    private String qxyzzdm;
    /**
     * 字段名：qxyzzqm
     * 备注: 
     * 默认值：无
     */
    private String qxyzzqm;
    /**
     * 字段名：yzqxrq
     * 备注: 
     * 默认值：无
     */
    private Timestamp yzqxrq;
    /**
     * 字段名：dzsqdbh
     * 备注: 
     * 默认值：无
     */
    private String dzsqdbh;
    /**
     * 字段名：ysbm
     * 备注: 
     * 默认值：无
     */
    private String ysbm;
    /**
     * 字段名：ysqm
     * 备注: 
     * 默认值：无
     */
    private String ysqm;
    /**
     * 字段名：gxsj
     * 备注: 
     * 默认值：无
     */
    private Timestamp gxsj;

    public HlhtMjzblMjzbl (){

    }

   /**
   * 字段名：yjlxh
   * get方法
   * 备注: 
   */
   public String getYjlxh(){

        return yjlxh;
   }

   /**
   * 字段名：yjlxh
   * set方法
   * 备注: 
   */
   public void setYjlxh(String yjlxh){
        this.yjlxh = yjlxh;
   }
   /**
   * 字段名：jzlsh
   * get方法
   * 备注: 
   */
   public String getJzlsh(){

        return jzlsh;
   }

   /**
   * 字段名：jzlsh
   * set方法
   * 备注: 
   */
   public void setJzlsh(String jzlsh){
        this.jzlsh = jzlsh;
   }
   /**
   * 字段名：patid
   * get方法
   * 备注: 
   */
   public String getPatid(){

        return patid;
   }

   /**
   * 字段名：patid
   * set方法
   * 备注: 
   */
   public void setPatid(String patid){
        this.patid = patid;
   }
   /**
   * 字段名：hzxm
   * get方法
   * 备注: 
   */
   public String getHzxm(){

        return hzxm;
   }

   /**
   * 字段名：hzxm
   * set方法
   * 备注: 
   */
   public void setHzxm(String hzxm){
        this.hzxm = hzxm;
   }
   /**
   * 字段名：mjzh
   * get方法
   * 备注: 
   */
   public String getMjzh(){

        return mjzh;
   }

   /**
   * 字段名：mjzh
   * set方法
   * 备注: 
   */
   public void setMjzh(String mjzh){
        this.mjzh = mjzh;
   }
   /**
   * 字段名：zyh
   * get方法
   * 备注: 
   */
   public String getZyh(){

        return zyh;
   }

   /**
   * 字段名：zyh
   * set方法
   * 备注: 
   */
   public void setZyh(String zyh){
        this.zyh = zyh;
   }
   /**
   * 字段名：sfzhm
   * get方法
   * 备注: 
   */
   public String getSfzhm(){

        return sfzhm;
   }

   /**
   * 字段名：sfzhm
   * set方法
   * 备注: 
   */
   public void setSfzhm(String sfzhm){
        this.sfzhm = sfzhm;
   }
   /**
   * 字段名：xbdm
   * get方法
   * 备注: 
   */
   public String getXbdm(){

        return xbdm;
   }

   /**
   * 字段名：xbdm
   * set方法
   * 备注: 
   */
   public void setXbdm(String xbdm){
        this.xbdm = xbdm;
   }
   /**
   * 字段名：xbmc
   * get方法
   * 备注: 
   */
   public String getXbmc(){

        return xbmc;
   }

   /**
   * 字段名：xbmc
   * set方法
   * 备注: 
   */
   public void setXbmc(String xbmc){
        this.xbmc = xbmc;
   }
   /**
   * 字段名：csrq
   * get方法
   * 备注: 
   */
   public Date getCsrq(){

        return csrq;
   }

   /**
   * 字段名：csrq
   * set方法
   * 备注: 
   */
   public void setCsrq(Date csrq){
        this.csrq = csrq;
   }
   /**
   * 字段名：nls
   * get方法
   * 备注: 
   */
   public Short getNls(){

        return nls;
   }

   /**
   * 字段名：nls
   * set方法
   * 备注: 
   */
   public void setNls(Short nls){
        this.nls = nls;
   }
   /**
   * 字段名：nly
   * get方法
   * 备注: 
   */
   public String getNly(){

        return nly;
   }

   /**
   * 字段名：nly
   * set方法
   * 备注: 
   */
   public void setNly(String nly){
        this.nly = nly;
   }
   /**
   * 字段名：zzjgdm
   * get方法
   * 备注: 
   */
   public String getZzjgdm(){

        return zzjgdm;
   }

   /**
   * 字段名：zzjgdm
   * set方法
   * 备注: 
   */
   public void setZzjgdm(String zzjgdm){
        this.zzjgdm = zzjgdm;
   }
   /**
   * 字段名：zzjgmc
   * get方法
   * 备注: 
   */
   public String getZzjgmc(){

        return zzjgmc;
   }

   /**
   * 字段名：zzjgmc
   * set方法
   * 备注: 
   */
   public void setZzjgmc(String zzjgmc){
        this.zzjgmc = zzjgmc;
   }
   /**
   * 字段名：ksdm
   * get方法
   * 备注: 
   */
   public String getKsdm(){

        return ksdm;
   }

   /**
   * 字段名：ksdm
   * set方法
   * 备注: 
   */
   public void setKsdm(String ksdm){
        this.ksdm = ksdm;
   }
   /**
   * 字段名：ksmc
   * get方法
   * 备注: 
   */
   public String getKsmc(){

        return ksmc;
   }

   /**
   * 字段名：ksmc
   * set方法
   * 备注: 
   */
   public void setKsmc(String ksmc){
        this.ksmc = ksmc;
   }
   /**
   * 字段名：gmsbz
   * get方法
   * 备注: 
   */
   public String getGmsbz(){

        return gmsbz;
   }

   /**
   * 字段名：gmsbz
   * set方法
   * 备注: 
   */
   public void setGmsbz(String gmsbz){
        this.gmsbz = gmsbz;
   }
   /**
   * 字段名：gms
   * get方法
   * 备注: 
   */
   public String getGms(){

        return gms;
   }

   /**
   * 字段名：gms
   * set方法
   * 备注: 
   */
   public void setGms(String gms){
        this.gms = gms;
   }
   /**
   * 字段名：jzrqsj
   * get方法
   * 备注: 
   */
   public Timestamp getJzrqsj(){

        return jzrqsj;
   }

   /**
   * 字段名：jzrqsj
   * set方法
   * 备注: 
   */
   public void setJzrqsj(Timestamp jzrqsj){
        this.jzrqsj = jzrqsj;
   }
   /**
   * 字段名：czbzdm
   * get方法
   * 备注: 
   */
   public String getCzbzdm(){

        return czbzdm;
   }

   /**
   * 字段名：czbzdm
   * set方法
   * 备注: 
   */
   public void setCzbzdm(String czbzdm){
        this.czbzdm = czbzdm;
   }
   /**
   * 字段名：czbzmc
   * get方法
   * 备注: 
   */
   public String getCzbzmc(){

        return czbzmc;
   }

   /**
   * 字段名：czbzmc
   * set方法
   * 备注: 
   */
   public void setCzbzmc(String czbzmc){
        this.czbzmc = czbzmc;
   }
   /**
   * 字段名：zs
   * get方法
   * 备注: 
   */
   public String getZs(){

        return zs;
   }

   /**
   * 字段名：zs
   * set方法
   * 备注: 
   */
   public void setZs(String zs){
        this.zs = zs;
   }
   /**
   * 字段名：xbs
   * get方法
   * 备注: 
   */
   public String getXbs(){

        return xbs;
   }

   /**
   * 字段名：xbs
   * set方法
   * 备注: 
   */
   public void setXbs(String xbs){
        this.xbs = xbs;
   }
   /**
   * 字段名：jws
   * get方法
   * 备注: 
   */
   public String getJws(){

        return jws;
   }

   /**
   * 字段名：jws
   * set方法
   * 备注: 
   */
   public void setJws(String jws){
        this.jws = jws;
   }
   /**
   * 字段名：tgjc
   * get方法
   * 备注: 
   */
   public String getTgjc(){

        return tgjc;
   }

   /**
   * 字段名：tgjc
   * set方法
   * 备注: 
   */
   public void setTgjc(String tgjc){
        this.tgjc = tgjc;
   }
   /**
   * 字段名：zyszgcjg
   * get方法
   * 备注: 
   */
   public String getZyszgcjg(){

        return zyszgcjg;
   }

   /**
   * 字段名：zyszgcjg
   * set方法
   * 备注: 
   */
   public void setZyszgcjg(String zyszgcjg){
        this.zyszgcjg = zyszgcjg;
   }
   /**
   * 字段名：fzjcxm
   * get方法
   * 备注: 
   */
   public String getFzjcxm(){

        return fzjcxm;
   }

   /**
   * 字段名：fzjcxm
   * set方法
   * 备注: 
   */
   public void setFzjcxm(String fzjcxm){
        this.fzjcxm = fzjcxm;
   }
   /**
   * 字段名：fzjcjg
   * get方法
   * 备注: 
   */
   public String getFzjcjg(){

        return fzjcjg;
   }

   /**
   * 字段名：fzjcjg
   * set方法
   * 备注: 
   */
   public void setFzjcjg(String fzjcjg){
        this.fzjcjg = fzjcjg;
   }
   /**
   * 字段名：xyzdbm
   * get方法
   * 备注: 
   */
   public String getXyzdbm(){

        return xyzdbm;
   }

   /**
   * 字段名：xyzdbm
   * set方法
   * 备注: 
   */
   public void setXyzdbm(String xyzdbm){
        this.xyzdbm = xyzdbm;
   }
   /**
   * 字段名：xyzdmc
   * get方法
   * 备注: 
   */
   public String getXyzdmc(){

        return xyzdmc;
   }

   /**
   * 字段名：xyzdmc
   * set方法
   * 备注: 
   */
   public void setXyzdmc(String xyzdmc){
        this.xyzdmc = xyzdmc;
   }
   /**
   * 字段名：zybmdm
   * get方法
   * 备注: 
   */
   public String getZybmdm(){

        return zybmdm;
   }

   /**
   * 字段名：zybmdm
   * set方法
   * 备注: 
   */
   public void setZybmdm(String zybmdm){
        this.zybmdm = zybmdm;
   }
   /**
   * 字段名：zybmmc
   * get方法
   * 备注: 
   */
   public String getZybmmc(){

        return zybmmc;
   }

   /**
   * 字段名：zybmmc
   * set方法
   * 备注: 
   */
   public void setZybmmc(String zybmmc){
        this.zybmmc = zybmmc;
   }
   /**
   * 字段名：zyzhdm
   * get方法
   * 备注: 
   */
   public String getZyzhdm(){

        return zyzhdm;
   }

   /**
   * 字段名：zyzhdm
   * set方法
   * 备注: 
   */
   public void setZyzhdm(String zyzhdm){
        this.zyzhdm = zyzhdm;
   }
   /**
   * 字段名：zyzhmc
   * get方法
   * 备注: 
   */
   public String getZyzhmc(){

        return zyzhmc;
   }

   /**
   * 字段名：zyzhmc
   * set方法
   * 备注: 
   */
   public void setZyzhmc(String zyzhmc){
        this.zyzhmc = zyzhmc;
   }
   /**
   * 字段名：bzyj
   * get方法
   * 备注: 
   */
   public String getBzyj(){

        return bzyj;
   }

   /**
   * 字段名：bzyj
   * set方法
   * 备注: 
   */
   public void setBzyj(String bzyj){
        this.bzyj = bzyj;
   }
   /**
   * 字段名：zfbm
   * get方法
   * 备注: 
   */
   public String getZfbm(){

        return zfbm;
   }

   /**
   * 字段名：zfbm
   * set方法
   * 备注: 
   */
   public void setZfbm(String zfbm){
        this.zfbm = zfbm;
   }
   /**
   * 字段名：zzzf
   * get方法
   * 备注: 
   */
   public String getZzzf(){

        return zzzf;
   }

   /**
   * 字段名：zzzf
   * set方法
   * 备注: 
   */
   public void setZzzf(String zzzf){
        this.zzzf = zzzf;
   }
   /**
   * 字段名：yzxmlx
   * get方法
   * 备注: 
   */
   public String getYzxmlx(){

        return yzxmlx;
   }

   /**
   * 字段名：yzxmlx
   * set方法
   * 备注: 
   */
   public void setYzxmlx(String yzxmlx){
        this.yzxmlx = yzxmlx;
   }
   /**
   * 字段名：yzxmlxmc
   * get方法
   * 备注: 
   */
   public String getYzxmlxmc(){

        return yzxmlxmc;
   }

   /**
   * 字段名：yzxmlxmc
   * set方法
   * 备注: 
   */
   public void setYzxmlxmc(String yzxmlxmc){
        this.yzxmlxmc = yzxmlxmc;
   }
   /**
   * 字段名：yzxmnr
   * get方法
   * 备注: 
   */
   public String getYzxmnr(){

        return yzxmnr;
   }

   /**
   * 字段名：yzxmnr
   * set方法
   * 备注: 
   */
   public void setYzxmnr(String yzxmnr){
        this.yzxmnr = yzxmnr;
   }
   /**
   * 字段名：yzbzxx
   * get方法
   * 备注: 
   */
   public String getYzbzxx(){

        return yzbzxx;
   }

   /**
   * 字段名：yzbzxx
   * set方法
   * 备注: 
   */
   public void setYzbzxx(String yzbzxx){
        this.yzbzxx = yzbzxx;
   }
   /**
   * 字段名：yzklksdm
   * get方法
   * 备注: 
   */
   public String getYzklksdm(){

        return yzklksdm;
   }

   /**
   * 字段名：yzklksdm
   * set方法
   * 备注: 
   */
   public void setYzklksdm(String yzklksdm){
        this.yzklksdm = yzklksdm;
   }
   /**
   * 字段名：yzklks
   * get方法
   * 备注: 
   */
   public String getYzklks(){

        return yzklks;
   }

   /**
   * 字段名：yzklks
   * set方法
   * 备注: 
   */
   public void setYzklks(String yzklks){
        this.yzklks = yzklks;
   }
   /**
   * 字段名：yzklzdm
   * get方法
   * 备注: 
   */
   public String getYzklzdm(){

        return yzklzdm;
   }

   /**
   * 字段名：yzklzdm
   * set方法
   * 备注: 
   */
   public void setYzklzdm(String yzklzdm){
        this.yzklzdm = yzklzdm;
   }
   /**
   * 字段名：yzklzqm
   * get方法
   * 备注: 
   */
   public String getYzklzqm(){

        return yzklzqm;
   }

   /**
   * 字段名：yzklzqm
   * set方法
   * 备注: 
   */
   public void setYzklzqm(String yzklzqm){
        this.yzklzqm = yzklzqm;
   }
   /**
   * 字段名：yzklrq
   * get方法
   * 备注: 
   */
   public Timestamp getYzklrq(){

        return yzklrq;
   }

   /**
   * 字段名：yzklrq
   * set方法
   * 备注: 
   */
   public void setYzklrq(Timestamp yzklrq){
        this.yzklrq = yzklrq;
   }
   /**
   * 字段名：yzshzdm
   * get方法
   * 备注: 
   */
   public String getYzshzdm(){

        return yzshzdm;
   }

   /**
   * 字段名：yzshzdm
   * set方法
   * 备注: 
   */
   public void setYzshzdm(String yzshzdm){
        this.yzshzdm = yzshzdm;
   }
   /**
   * 字段名：yzshzqm
   * get方法
   * 备注: 
   */
   public String getYzshzqm(){

        return yzshzqm;
   }

   /**
   * 字段名：yzshzqm
   * set方法
   * 备注: 
   */
   public void setYzshzqm(String yzshzqm){
        this.yzshzqm = yzshzqm;
   }
   /**
   * 字段名：yzshrq
   * get方法
   * 备注: 
   */
   public Timestamp getYzshrq(){

        return yzshrq;
   }

   /**
   * 字段名：yzshrq
   * set方法
   * 备注: 
   */
   public void setYzshrq(Timestamp yzshrq){
        this.yzshrq = yzshrq;
   }
   /**
   * 字段名：zxksdm
   * get方法
   * 备注: 
   */
   public String getZxksdm(){

        return zxksdm;
   }

   /**
   * 字段名：zxksdm
   * set方法
   * 备注: 
   */
   public void setZxksdm(String zxksdm){
        this.zxksdm = zxksdm;
   }
   /**
   * 字段名：yzzxks
   * get方法
   * 备注: 
   */
   public String getYzzxks(){

        return yzzxks;
   }

   /**
   * 字段名：yzzxks
   * set方法
   * 备注: 
   */
   public void setYzzxks(String yzzxks){
        this.yzzxks = yzzxks;
   }
   /**
   * 字段名：yzzxzdm
   * get方法
   * 备注: 
   */
   public String getYzzxzdm(){

        return yzzxzdm;
   }

   /**
   * 字段名：yzzxzdm
   * set方法
   * 备注: 
   */
   public void setYzzxzdm(String yzzxzdm){
        this.yzzxzdm = yzzxzdm;
   }
   /**
   * 字段名：yzzxzqm
   * get方法
   * 备注: 
   */
   public String getYzzxzqm(){

        return yzzxzqm;
   }

   /**
   * 字段名：yzzxzqm
   * set方法
   * 备注: 
   */
   public void setYzzxzqm(String yzzxzqm){
        this.yzzxzqm = yzzxzqm;
   }
   /**
   * 字段名：yzzxrq
   * get方法
   * 备注: 
   */
   public Timestamp getYzzxrq(){

        return yzzxrq;
   }

   /**
   * 字段名：yzzxrq
   * set方法
   * 备注: 
   */
   public void setYzzxrq(Timestamp yzzxrq){
        this.yzzxrq = yzzxrq;
   }
   /**
   * 字段名：yzzxzt
   * get方法
   * 备注: 
   */
   public String getYzzxzt(){

        return yzzxzt;
   }

   /**
   * 字段名：yzzxzt
   * set方法
   * 备注: 
   */
   public void setYzzxzt(String yzzxzt){
        this.yzzxzt = yzzxzt;
   }
   /**
   * 字段名：qxyzzdm
   * get方法
   * 备注: 
   */
   public String getQxyzzdm(){

        return qxyzzdm;
   }

   /**
   * 字段名：qxyzzdm
   * set方法
   * 备注: 
   */
   public void setQxyzzdm(String qxyzzdm){
        this.qxyzzdm = qxyzzdm;
   }
   /**
   * 字段名：qxyzzqm
   * get方法
   * 备注: 
   */
   public String getQxyzzqm(){

        return qxyzzqm;
   }

   /**
   * 字段名：qxyzzqm
   * set方法
   * 备注: 
   */
   public void setQxyzzqm(String qxyzzqm){
        this.qxyzzqm = qxyzzqm;
   }
   /**
   * 字段名：yzqxrq
   * get方法
   * 备注: 
   */
   public Timestamp getYzqxrq(){

        return yzqxrq;
   }

   /**
   * 字段名：yzqxrq
   * set方法
   * 备注: 
   */
   public void setYzqxrq(Timestamp yzqxrq){
        this.yzqxrq = yzqxrq;
   }
   /**
   * 字段名：dzsqdbh
   * get方法
   * 备注: 
   */
   public String getDzsqdbh(){

        return dzsqdbh;
   }

   /**
   * 字段名：dzsqdbh
   * set方法
   * 备注: 
   */
   public void setDzsqdbh(String dzsqdbh){
        this.dzsqdbh = dzsqdbh;
   }
   /**
   * 字段名：ysbm
   * get方法
   * 备注: 
   */
   public String getYsbm(){

        return ysbm;
   }

   /**
   * 字段名：ysbm
   * set方法
   * 备注: 
   */
   public void setYsbm(String ysbm){
        this.ysbm = ysbm;
   }
   /**
   * 字段名：ysqm
   * get方法
   * 备注: 
   */
   public String getYsqm(){

        return ysqm;
   }

   /**
   * 字段名：ysqm
   * set方法
   * 备注: 
   */
   public void setYsqm(String ysqm){
        this.ysqm = ysqm;
   }
   /**
   * 字段名：gxsj
   * get方法
   * 备注: 
   */
   public Timestamp getGxsj(){

        return gxsj;
   }

   /**
   * 字段名：gxsj
   * set方法
   * 备注: 
   */
   public void setGxsj(Timestamp gxsj){
        this.gxsj = gxsj;
   }

}