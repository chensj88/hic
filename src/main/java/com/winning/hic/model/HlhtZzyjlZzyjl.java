package com.winning.hic.model;

import java.util.Date; 
import java.util.Date; 
import java.util.Date; 
import java.util.Date; 
import java.util.Date; 
import java.io.Serializable; 

import org.apache.ibatis.type.Alias; 

import com.winning.hic.model.BaseDomain;



/**
 * @author HLHT
 * @title 
 * @email Winning Health
 * @package com.winning.hic.model
 * @date 2018-35-31 16:35:09
 */
@Alias("hlhtZzyjlZzyjl")
public class HlhtZzyjlZzyjl extends BaseDomain implements Serializable {

    private static final long serialVersionUID = -1L;

    /**
     * 字段名：yjlxh
     * 备注: 
     * 默认值：无
     */
    private String yjlxh;
    /**
     * 字段名：jzlsh
     * 备注: 
     * 默认值：无
     */
    private String jzlsh;
    /**
     * 字段名：patid
     * 备注: 
     * 默认值：无
     */
    private String patid;
    /**
     * 字段名：jkkh
     * 备注: 
     * 默认值：无
     */
    private String jkkh;
    /**
     * 字段名：mjzh
     * 备注: 
     * 默认值：无
     */
    private String mjzh;
    /**
     * 字段名：zyh
     * 备注: 
     * 默认值：无
     */
    private String zyh;
    /**
     * 字段名：jzlb
     * 备注: 
     * 默认值：无
     */
    private String jzlb;
    /**
     * 字段名：hzxm
     * 备注: 
     * 默认值：无
     */
    private String hzxm;
    /**
     * 字段名：sfzhm
     * 备注: 
     * 默认值：无
     */
    private String sfzhm;
    /**
     * 字段名：xbdm
     * 备注: 
     * 默认值：无
     */
    private String xbdm;
    /**
     * 字段名：xbmc
     * 备注: 
     * 默认值：无
     */
    private String xbmc;
    /**
     * 字段名：csrq
     * 备注: 
     * 默认值：无
     */
    private Date csrq;
    /**
     * 字段名：gms
     * 备注: 
     * 默认值：无
     */
    private String gms;
    /**
     * 字段名：jbs
     * 备注: 
     * 默认值：无
     */
    private String jbs;
    /**
     * 字段名：sss
     * 备注: 
     * 默认值：无
     */
    private String sss;
    /**
     * 字段名：zzrq
     * 备注: 
     * 默认值：无
     */
    private Date zzrq;
    /**
     * 字段名：zzyy
     * 备注: 
     * 默认值：无
     */
    private String zzyy;
    /**
     * 字段名：zzjl
     * 备注: 
     * 默认值：无
     */
    private String zzjl;
    /**
     * 字段名：ysbm
     * 备注: 
     * 默认值：无
     */
    private String ysbm;
    /**
     * 字段名：zzysxm
     * 备注: 
     * 默认值：无
     */
    private String zzysxm;
    /**
     * 字段名：jbzdbm
     * 备注: 
     * 默认值：无
     */
    private String jbzdbm;
    /**
     * 字段名：jbzd
     * 备注: 
     * 默认值：无
     */
    private String jbzd;
    /**
     * 字段名：zdrq
     * 备注: 
     * 默认值：无
     */
    private Date zdrq;
    /**
     * 字段名：zdyljgdm
     * 备注: 
     * 默认值：无
     */
    private String zdyljgdm;
    /**
     * 字段名：zdjgmc
     * 备注: 
     * 默认值：无
     */
    private String zdjgmc;
    /**
     * 字段名：xgzz
     * 备注: 
     * 默认值：无
     */
    private String xgzz;
    /**
     * 字段名：zyzlcs
     * 备注: 
     * 默认值：无
     */
    private String zyzlcs;
    /**
     * 字段名：fzjcjg
     * 备注: 
     * 默认值：无
     */
    private String fzjcjg;
    /**
     * 字段名：ssczbm
     * 备注: 
     * 默认值：无
     */
    private String ssczbm;
    /**
     * 字段名：ssjczmc
     * 备注: 
     * 默认值：无
     */
    private String ssjczmc;
    /**
     * 字段名：ssczksrq
     * 备注: 
     * 默认值：无
     */
    private Date ssczksrq;
    /**
     * 字段名：zlfa
     * 备注: 
     * 默认值：无
     */
    private String zlfa;
    /**
     * 字段名：czjh
     * 备注: 
     * 默认值：无
     */
    private String czjh;
    /**
     * 字段名：jkwtpg
     * 备注: 
     * 默认值：无
     */
    private String jkwtpg;
    /**
     * 字段名：kfcszd
     * 备注: 
     * 默认值：无
     */
    private String kfcszd;
    /**
     * 字段名：zcjgdm
     * 备注: 
     * 默认值：无
     */
    private String zcjgdm;
    /**
     * 字段名：zcjgmc
     * 备注: 
     * 默认值：无
     */
    private String zcjgmc;
    /**
     * 字段名：zcjgksdm
     * 备注: 
     * 默认值：无
     */
    private String zcjgksdm;
    /**
     * 字段名：zcjgksmc
     * 备注: 
     * 默认值：无
     */
    private String zcjgksmc;
    /**
     * 字段名：zryljgdm
     * 备注: 
     * 默认值：无
     */
    private String zryljgdm;
    /**
     * 字段名：zrjgmc
     * 备注: 
     * 默认值：无
     */
    private String zrjgmc;
    /**
     * 字段名：zrjgksdm
     * 备注: 
     * 默认值：无
     */
    private String zrjgksdm;
    /**
     * 字段名：zrjgksmc
     * 备注: 
     * 默认值：无
     */
    private String zrjgksmc;
    /**
     * 字段名：gxsj
     * 备注: 
     * 默认值：无
     */
    private Date gxsj;

    public HlhtZzyjlZzyjl (){

    }

   /**
   * 字段名：yjlxh
   * get方法
   * 备注: 
   */
   public String getYjlxh(){

        return yjlxh;
   }

   /**
   * 字段名：yjlxh
   * set方法
   * 备注: 
   */
   public void setYjlxh(String yjlxh){
        this.yjlxh = yjlxh;
   }
   /**
   * 字段名：jzlsh
   * get方法
   * 备注: 
   */
   public String getJzlsh(){

        return jzlsh;
   }

   /**
   * 字段名：jzlsh
   * set方法
   * 备注: 
   */
   public void setJzlsh(String jzlsh){
        this.jzlsh = jzlsh;
   }
   /**
   * 字段名：patid
   * get方法
   * 备注: 
   */
   public String getPatid(){

        return patid;
   }

   /**
   * 字段名：patid
   * set方法
   * 备注: 
   */
   public void setPatid(String patid){
        this.patid = patid;
   }
   /**
   * 字段名：jkkh
   * get方法
   * 备注: 
   */
   public String getJkkh(){

        return jkkh;
   }

   /**
   * 字段名：jkkh
   * set方法
   * 备注: 
   */
   public void setJkkh(String jkkh){
        this.jkkh = jkkh;
   }
   /**
   * 字段名：mjzh
   * get方法
   * 备注: 
   */
   public String getMjzh(){

        return mjzh;
   }

   /**
   * 字段名：mjzh
   * set方法
   * 备注: 
   */
   public void setMjzh(String mjzh){
        this.mjzh = mjzh;
   }
   /**
   * 字段名：zyh
   * get方法
   * 备注: 
   */
   public String getZyh(){

        return zyh;
   }

   /**
   * 字段名：zyh
   * set方法
   * 备注: 
   */
   public void setZyh(String zyh){
        this.zyh = zyh;
   }
   /**
   * 字段名：jzlb
   * get方法
   * 备注: 
   */
   public String getJzlb(){

        return jzlb;
   }

   /**
   * 字段名：jzlb
   * set方法
   * 备注: 
   */
   public void setJzlb(String jzlb){
        this.jzlb = jzlb;
   }
   /**
   * 字段名：hzxm
   * get方法
   * 备注: 
   */
   public String getHzxm(){

        return hzxm;
   }

   /**
   * 字段名：hzxm
   * set方法
   * 备注: 
   */
   public void setHzxm(String hzxm){
        this.hzxm = hzxm;
   }
   /**
   * 字段名：sfzhm
   * get方法
   * 备注: 
   */
   public String getSfzhm(){

        return sfzhm;
   }

   /**
   * 字段名：sfzhm
   * set方法
   * 备注: 
   */
   public void setSfzhm(String sfzhm){
        this.sfzhm = sfzhm;
   }
   /**
   * 字段名：xbdm
   * get方法
   * 备注: 
   */
   public String getXbdm(){

        return xbdm;
   }

   /**
   * 字段名：xbdm
   * set方法
   * 备注: 
   */
   public void setXbdm(String xbdm){
        this.xbdm = xbdm;
   }
   /**
   * 字段名：xbmc
   * get方法
   * 备注: 
   */
   public String getXbmc(){

        return xbmc;
   }

   /**
   * 字段名：xbmc
   * set方法
   * 备注: 
   */
   public void setXbmc(String xbmc){
        this.xbmc = xbmc;
   }
   /**
   * 字段名：csrq
   * get方法
   * 备注: 
   */
   public Date getCsrq(){

        return csrq;
   }

   /**
   * 字段名：csrq
   * set方法
   * 备注: 
   */
   public void setCsrq(Date csrq){
        this.csrq = csrq;
   }
   /**
   * 字段名：gms
   * get方法
   * 备注: 
   */
   public String getGms(){

        return gms;
   }

   /**
   * 字段名：gms
   * set方法
   * 备注: 
   */
   public void setGms(String gms){
        this.gms = gms;
   }
   /**
   * 字段名：jbs
   * get方法
   * 备注: 
   */
   public String getJbs(){

        return jbs;
   }

   /**
   * 字段名：jbs
   * set方法
   * 备注: 
   */
   public void setJbs(String jbs){
        this.jbs = jbs;
   }
   /**
   * 字段名：sss
   * get方法
   * 备注: 
   */
   public String getSss(){

        return sss;
   }

   /**
   * 字段名：sss
   * set方法
   * 备注: 
   */
   public void setSss(String sss){
        this.sss = sss;
   }
   /**
   * 字段名：zzrq
   * get方法
   * 备注: 
   */
   public Date getZzrq(){

        return zzrq;
   }

   /**
   * 字段名：zzrq
   * set方法
   * 备注: 
   */
   public void setZzrq(Date zzrq){
        this.zzrq = zzrq;
   }
   /**
   * 字段名：zzyy
   * get方法
   * 备注: 
   */
   public String getZzyy(){

        return zzyy;
   }

   /**
   * 字段名：zzyy
   * set方法
   * 备注: 
   */
   public void setZzyy(String zzyy){
        this.zzyy = zzyy;
   }
   /**
   * 字段名：zzjl
   * get方法
   * 备注: 
   */
   public String getZzjl(){

        return zzjl;
   }

   /**
   * 字段名：zzjl
   * set方法
   * 备注: 
   */
   public void setZzjl(String zzjl){
        this.zzjl = zzjl;
   }
   /**
   * 字段名：ysbm
   * get方法
   * 备注: 
   */
   public String getYsbm(){

        return ysbm;
   }

   /**
   * 字段名：ysbm
   * set方法
   * 备注: 
   */
   public void setYsbm(String ysbm){
        this.ysbm = ysbm;
   }
   /**
   * 字段名：zzysxm
   * get方法
   * 备注: 
   */
   public String getZzysxm(){

        return zzysxm;
   }

   /**
   * 字段名：zzysxm
   * set方法
   * 备注: 
   */
   public void setZzysxm(String zzysxm){
        this.zzysxm = zzysxm;
   }
   /**
   * 字段名：jbzdbm
   * get方法
   * 备注: 
   */
   public String getJbzdbm(){

        return jbzdbm;
   }

   /**
   * 字段名：jbzdbm
   * set方法
   * 备注: 
   */
   public void setJbzdbm(String jbzdbm){
        this.jbzdbm = jbzdbm;
   }
   /**
   * 字段名：jbzd
   * get方法
   * 备注: 
   */
   public String getJbzd(){

        return jbzd;
   }

   /**
   * 字段名：jbzd
   * set方法
   * 备注: 
   */
   public void setJbzd(String jbzd){
        this.jbzd = jbzd;
   }
   /**
   * 字段名：zdrq
   * get方法
   * 备注: 
   */
   public Date getZdrq(){

        return zdrq;
   }

   /**
   * 字段名：zdrq
   * set方法
   * 备注: 
   */
   public void setZdrq(Date zdrq){
        this.zdrq = zdrq;
   }
   /**
   * 字段名：zdyljgdm
   * get方法
   * 备注: 
   */
   public String getZdyljgdm(){

        return zdyljgdm;
   }

   /**
   * 字段名：zdyljgdm
   * set方法
   * 备注: 
   */
   public void setZdyljgdm(String zdyljgdm){
        this.zdyljgdm = zdyljgdm;
   }
   /**
   * 字段名：zdjgmc
   * get方法
   * 备注: 
   */
   public String getZdjgmc(){

        return zdjgmc;
   }

   /**
   * 字段名：zdjgmc
   * set方法
   * 备注: 
   */
   public void setZdjgmc(String zdjgmc){
        this.zdjgmc = zdjgmc;
   }
   /**
   * 字段名：xgzz
   * get方法
   * 备注: 
   */
   public String getXgzz(){

        return xgzz;
   }

   /**
   * 字段名：xgzz
   * set方法
   * 备注: 
   */
   public void setXgzz(String xgzz){
        this.xgzz = xgzz;
   }
   /**
   * 字段名：zyzlcs
   * get方法
   * 备注: 
   */
   public String getZyzlcs(){

        return zyzlcs;
   }

   /**
   * 字段名：zyzlcs
   * set方法
   * 备注: 
   */
   public void setZyzlcs(String zyzlcs){
        this.zyzlcs = zyzlcs;
   }
   /**
   * 字段名：fzjcjg
   * get方法
   * 备注: 
   */
   public String getFzjcjg(){

        return fzjcjg;
   }

   /**
   * 字段名：fzjcjg
   * set方法
   * 备注: 
   */
   public void setFzjcjg(String fzjcjg){
        this.fzjcjg = fzjcjg;
   }
   /**
   * 字段名：ssczbm
   * get方法
   * 备注: 
   */
   public String getSsczbm(){

        return ssczbm;
   }

   /**
   * 字段名：ssczbm
   * set方法
   * 备注: 
   */
   public void setSsczbm(String ssczbm){
        this.ssczbm = ssczbm;
   }
   /**
   * 字段名：ssjczmc
   * get方法
   * 备注: 
   */
   public String getSsjczmc(){

        return ssjczmc;
   }

   /**
   * 字段名：ssjczmc
   * set方法
   * 备注: 
   */
   public void setSsjczmc(String ssjczmc){
        this.ssjczmc = ssjczmc;
   }
   /**
   * 字段名：ssczksrq
   * get方法
   * 备注: 
   */
   public Date getSsczksrq(){

        return ssczksrq;
   }

   /**
   * 字段名：ssczksrq
   * set方法
   * 备注: 
   */
   public void setSsczksrq(Date ssczksrq){
        this.ssczksrq = ssczksrq;
   }
   /**
   * 字段名：zlfa
   * get方法
   * 备注: 
   */
   public String getZlfa(){

        return zlfa;
   }

   /**
   * 字段名：zlfa
   * set方法
   * 备注: 
   */
   public void setZlfa(String zlfa){
        this.zlfa = zlfa;
   }
   /**
   * 字段名：czjh
   * get方法
   * 备注: 
   */
   public String getCzjh(){

        return czjh;
   }

   /**
   * 字段名：czjh
   * set方法
   * 备注: 
   */
   public void setCzjh(String czjh){
        this.czjh = czjh;
   }
   /**
   * 字段名：jkwtpg
   * get方法
   * 备注: 
   */
   public String getJkwtpg(){

        return jkwtpg;
   }

   /**
   * 字段名：jkwtpg
   * set方法
   * 备注: 
   */
   public void setJkwtpg(String jkwtpg){
        this.jkwtpg = jkwtpg;
   }
   /**
   * 字段名：kfcszd
   * get方法
   * 备注: 
   */
   public String getKfcszd(){

        return kfcszd;
   }

   /**
   * 字段名：kfcszd
   * set方法
   * 备注: 
   */
   public void setKfcszd(String kfcszd){
        this.kfcszd = kfcszd;
   }
   /**
   * 字段名：zcjgdm
   * get方法
   * 备注: 
   */
   public String getZcjgdm(){

        return zcjgdm;
   }

   /**
   * 字段名：zcjgdm
   * set方法
   * 备注: 
   */
   public void setZcjgdm(String zcjgdm){
        this.zcjgdm = zcjgdm;
   }
   /**
   * 字段名：zcjgmc
   * get方法
   * 备注: 
   */
   public String getZcjgmc(){

        return zcjgmc;
   }

   /**
   * 字段名：zcjgmc
   * set方法
   * 备注: 
   */
   public void setZcjgmc(String zcjgmc){
        this.zcjgmc = zcjgmc;
   }
   /**
   * 字段名：zcjgksdm
   * get方法
   * 备注: 
   */
   public String getZcjgksdm(){

        return zcjgksdm;
   }

   /**
   * 字段名：zcjgksdm
   * set方法
   * 备注: 
   */
   public void setZcjgksdm(String zcjgksdm){
        this.zcjgksdm = zcjgksdm;
   }
   /**
   * 字段名：zcjgksmc
   * get方法
   * 备注: 
   */
   public String getZcjgksmc(){

        return zcjgksmc;
   }

   /**
   * 字段名：zcjgksmc
   * set方法
   * 备注: 
   */
   public void setZcjgksmc(String zcjgksmc){
        this.zcjgksmc = zcjgksmc;
   }
   /**
   * 字段名：zryljgdm
   * get方法
   * 备注: 
   */
   public String getZryljgdm(){

        return zryljgdm;
   }

   /**
   * 字段名：zryljgdm
   * set方法
   * 备注: 
   */
   public void setZryljgdm(String zryljgdm){
        this.zryljgdm = zryljgdm;
   }
   /**
   * 字段名：zrjgmc
   * get方法
   * 备注: 
   */
   public String getZrjgmc(){

        return zrjgmc;
   }

   /**
   * 字段名：zrjgmc
   * set方法
   * 备注: 
   */
   public void setZrjgmc(String zrjgmc){
        this.zrjgmc = zrjgmc;
   }
   /**
   * 字段名：zrjgksdm
   * get方法
   * 备注: 
   */
   public String getZrjgksdm(){

        return zrjgksdm;
   }

   /**
   * 字段名：zrjgksdm
   * set方法
   * 备注: 
   */
   public void setZrjgksdm(String zrjgksdm){
        this.zrjgksdm = zrjgksdm;
   }
   /**
   * 字段名：zrjgksmc
   * get方法
   * 备注: 
   */
   public String getZrjgksmc(){

        return zrjgksmc;
   }

   /**
   * 字段名：zrjgksmc
   * set方法
   * 备注: 
   */
   public void setZrjgksmc(String zrjgksmc){
        this.zrjgksmc = zrjgksmc;
   }
   /**
   * 字段名：gxsj
   * get方法
   * 备注: 
   */
   public Date getGxsj(){

        return gxsj;
   }

   /**
   * 字段名：gxsj
   * set方法
   * 备注: 
   */
   public void setGxsj(Date gxsj){
        this.gxsj = gxsj;
   }

}