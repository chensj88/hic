package com.winning.hic.model;

import java.util.Date; 
import java.util.Date; 
import java.util.Date; 
import java.util.Date; 
import java.util.Date; 
import java.io.Serializable; 

import org.apache.ibatis.type.Alias; 

import com.winning.hic.model.BaseDomain;



/**
 * @author HLHT
 * @title 
 * @email Winning Health
 * @package com.winning.hic.model
 * @date 2018-29-31 16:29:18
 */
@Alias("hlhtCyxjCyxj")
public class HlhtCyxjCyxj extends BaseDomain implements Serializable {

    private static final long serialVersionUID = -1L;

    /**
     * 字段名：yjlxh
     * 备注: 
     * 默认值：无
     */
    private String yjlxh;
    /**
     * 字段名：jzlsh
     * 备注: 
     * 默认值：无
     */
    private String jzlsh;
    /**
     * 字段名：patid
     * 备注: 
     * 默认值：无
     */
    private String patid;
    /**
     * 字段名：zyh
     * 备注: 
     * 默认值：无
     */
    private String zyh;
    /**
     * 字段名：zzjgdm
     * 备注: 
     * 默认值：无
     */
    private String zzjgdm;
    /**
     * 字段名：zzjgmc
     * 备注: 
     * 默认值：无
     */
    private String zzjgmc;
    /**
     * 字段名：jkkh
     * 备注: 
     * 默认值：无
     */
    private String jkkh;
    /**
     * 字段名：ksdm
     * 备注: 
     * 默认值：无
     */
    private String ksdm;
    /**
     * 字段名：ksmc
     * 备注: 
     * 默认值：无
     */
    private String ksmc;
    /**
     * 字段名：bqdm
     * 备注: 
     * 默认值：无
     */
    private String bqdm;
    /**
     * 字段名：bqmc
     * 备注: 
     * 默认值：无
     */
    private String bqmc;
    /**
     * 字段名：bfh
     * 备注: 
     * 默认值：无
     */
    private String bfh;
    /**
     * 字段名：bch
     * 备注: 
     * 默认值：无
     */
    private String bch;
    /**
     * 字段名：hzxm
     * 备注: 
     * 默认值：无
     */
    private String hzxm;
    /**
     * 字段名：sfzhm
     * 备注: 
     * 默认值：无
     */
    private String sfzhm;
    /**
     * 字段名：xbdm
     * 备注: 
     * 默认值：无
     */
    private String xbdm;
    /**
     * 字段名：xbmc
     * 备注: 
     * 默认值：无
     */
    private String xbmc;
    /**
     * 字段名：nls
     * 备注: 
     * 默认值：无
     */
    private Short nls;
    /**
     * 字段名：nly
     * 备注: 
     * 默认值：无
     */
    private String nly;
    /**
     * 字段名：hyzkdm
     * 备注: 
     * 默认值：无
     */
    private String hyzkdm;
    /**
     * 字段名：hyzkmc
     * 备注: 
     * 默认值：无
     */
    private String hyzkmc;
    /**
     * 字段名：zylbdm
     * 备注: 
     * 默认值：无
     */
    private String zylbdm;
    /**
     * 字段名：zylbmc
     * 备注: 
     * 默认值：无
     */
    private String zylbmc;
    /**
     * 字段名：hzdhhm
     * 备注: 
     * 默认值：无
     */
    private String hzdhhm;
    /**
     * 字段名：zzlbdm
     * 备注: 
     * 默认值：无
     */
    private String zzlbdm;
    /**
     * 字段名：zzlbmc
     * 备注: 
     * 默认值：无
     */
    private String zzlbmc;
    /**
     * 字段名：dzsf
     * 备注: 
     * 默认值：无
     */
    private String dzsf;
    /**
     * 字段名：dzsq
     * 备注: 
     * 默认值：无
     */
    private String dzsq;
    /**
     * 字段名：dzx
     * 备注: 
     * 默认值：无
     */
    private String dzx;
    /**
     * 字段名：dzxz
     * 备注: 
     * 默认值：无
     */
    private String dzxz;
    /**
     * 字段名：dzc
     * 备注: 
     * 默认值：无
     */
    private String dzc;
    /**
     * 字段名：dzmphm
     * 备注: 
     * 默认值：无
     */
    private String dzmphm;
    /**
     * 字段名：yzbm
     * 备注: 
     * 默认值：无
     */
    private String yzbm;
    /**
     * 字段名：lxrxm
     * 备注: 
     * 默认值：无
     */
    private String lxrxm;
    /**
     * 字段名：lxrdhhm
     * 备注: 
     * 默认值：无
     */
    private String lxrdhhm;
    /**
     * 字段名：ryrq
     * 备注: 
     * 默认值：无
     */
    private Date ryrq;
    /**
     * 字段名：cyrq
     * 备注: 
     * 默认值：无
     */
    private Date cyrq;
    /**
     * 字段名：sjzyts
     * 备注: 
     * 默认值：无
     */
    private Integer sjzyts;
    /**
     * 字段名：ryqk
     * 备注: 
     * 默认值：无
     */
    private String ryqk;
    /**
     * 字段名：yxfzjcjg
     * 备注: 
     * 默认值：无
     */
    private String yxfzjcjg;
    /**
     * 字段名：zyszgcjg
     * 备注: 
     * 默认值：无
     */
    private String zyszgcjg;
    /**
     * 字段名：rzxyzdbm
     * 备注: 
     * 默认值：无
     */
    private String rzxyzdbm;
    /**
     * 字段名：rzxyzd
     * 备注: 
     * 默认值：无
     */
    private String rzxyzd;
    /**
     * 字段名：rzzybmdm
     * 备注: 
     * 默认值：无
     */
    private String rzzybmdm;
    /**
     * 字段名：rzzybm
     * 备注: 
     * 默认值：无
     */
    private String rzzybm;
    /**
     * 字段名：rzzyzhdm
     * 备注: 
     * 默认值：无
     */
    private String rzzyzhdm;
    /**
     * 字段名：rzzyzh
     * 备注: 
     * 默认值：无
     */
    private String rzzyzh;
    /**
     * 字段名：czxyzdbm
     * 备注: 
     * 默认值：无
     */
    private String czxyzdbm;
    /**
     * 字段名：czxyzd
     * 备注: 
     * 默认值：无
     */
    private String czxyzd;
    /**
     * 字段名：czzybmdm
     * 备注: 
     * 默认值：无
     */
    private String czzybmdm;
    /**
     * 字段名：czzybm
     * 备注: 
     * 默认值：无
     */
    private String czzybm;
    /**
     * 字段名：czzyzhdm
     * 备注: 
     * 默认值：无
     */
    private String czzyzhdm;
    /**
     * 字段名：czzyzh
     * 备注: 
     * 默认值：无
     */
    private String czzyzh;
    /**
     * 字段名：ssqklbdm
     * 备注: 
     * 默认值：无
     */
    private String ssqklbdm;
    /**
     * 字段名：ssqklbmc
     * 备注: 
     * 默认值：无
     */
    private String ssqklbmc;
    /**
     * 字段名：qkyhdjdm
     * 备注: 
     * 默认值：无
     */
    private String qkyhdjdm;
    /**
     * 字段名：qkyhdjmc
     * 备注: 
     * 默认值：无
     */
    private String qkyhdjmc;
    /**
     * 字段名：ssczbm
     * 备注: 
     * 默认值：无
     */
    private String ssczbm;
    /**
     * 字段名：ssjczmc
     * 备注: 
     * 默认值：无
     */
    private String ssjczmc;
    /**
     * 字段名：ssczksrq
     * 备注: 
     * 默认值：无
     */
    private Date ssczksrq;
    /**
     * 字段名：mzffdm
     * 备注: 
     * 默认值：无
     */
    private String mzffdm;
    /**
     * 字段名：mzffmc
     * 备注: 
     * 默认值：无
     */
    private String mzffmc;
    /**
     * 字段名：ssgc
     * 备注: 
     * 默认值：无
     */
    private String ssgc;
    /**
     * 字段名：zfbm
     * 备注: 
     * 默认值：无
     */
    private String zfbm;
    /**
     * 字段名：zzzf
     * 备注: 
     * 默认值：无
     */
    private String zzzf;
    /**
     * 字段名：zyjzff
     * 备注: 
     * 默认值：无
     */
    private String zyjzff;
    /**
     * 字段名：zyyyff
     * 备注: 
     * 默认值：无
     */
    private String zyyyff;
    /**
     * 字段名：zlgcms
     * 备注: 
     * 默认值：无
     */
    private String zlgcms;
    /**
     * 字段名：cyqk
     * 备注: 
     * 默认值：无
     */
    private String cyqk;
    /**
     * 字段名：cyzztz
     * 备注: 
     * 默认值：无
     */
    private String cyzztz;
    /**
     * 字段名：cyyz
     * 备注: 
     * 默认值：无
     */
    private String cyyz;
    /**
     * 字段名：zljgdm
     * 备注: 
     * 默认值：无
     */
    private String zljgdm;
    /**
     * 字段名：zljgmc
     * 备注: 
     * 默认值：无
     */
    private String zljgmc;
    /**
     * 字段名：zyysbm
     * 备注: 
     * 默认值：无
     */
    private String zyysbm;
    /**
     * 字段名：zyysqm
     * 备注: 
     * 默认值：无
     */
    private String zyysqm;
    /**
     * 字段名：sjysbm
     * 备注: 
     * 默认值：无
     */
    private String sjysbm;
    /**
     * 字段名：sjysqm
     * 备注: 
     * 默认值：无
     */
    private String sjysqm;
    /**
     * 字段名：qmrq
     * 备注: 
     * 默认值：无
     */
    private Date qmrq;
    /**
     * 字段名：gxsj
     * 备注: 
     * 默认值：无
     */
    private Date gxsj;
    /**
     * 字段名：syxh
     * 备注: 
     * 默认值：无
     */
    private String syxh;
    /**
     * 字段名：tjzt
     * 备注: 
     * 默认值：无
     */
    private String tjzt;
    /**
     * 字段名：yxjl
     * 备注: 
     * 默认值：无
     */
    private Integer yxjl;

    public HlhtCyxjCyxj (){

    }

   /**
   * 字段名：yjlxh
   * get方法
   * 备注: 
   */
   public String getYjlxh(){

        return yjlxh;
   }

   /**
   * 字段名：yjlxh
   * set方法
   * 备注: 
   */
   public void setYjlxh(String yjlxh){
        this.yjlxh = yjlxh;
   }
   /**
   * 字段名：jzlsh
   * get方法
   * 备注: 
   */
   public String getJzlsh(){

        return jzlsh;
   }

   /**
   * 字段名：jzlsh
   * set方法
   * 备注: 
   */
   public void setJzlsh(String jzlsh){
        this.jzlsh = jzlsh;
   }
   /**
   * 字段名：patid
   * get方法
   * 备注: 
   */
   public String getPatid(){

        return patid;
   }

   /**
   * 字段名：patid
   * set方法
   * 备注: 
   */
   public void setPatid(String patid){
        this.patid = patid;
   }
   /**
   * 字段名：zyh
   * get方法
   * 备注: 
   */
   public String getZyh(){

        return zyh;
   }

   /**
   * 字段名：zyh
   * set方法
   * 备注: 
   */
   public void setZyh(String zyh){
        this.zyh = zyh;
   }
   /**
   * 字段名：zzjgdm
   * get方法
   * 备注: 
   */
   public String getZzjgdm(){

        return zzjgdm;
   }

   /**
   * 字段名：zzjgdm
   * set方法
   * 备注: 
   */
   public void setZzjgdm(String zzjgdm){
        this.zzjgdm = zzjgdm;
   }
   /**
   * 字段名：zzjgmc
   * get方法
   * 备注: 
   */
   public String getZzjgmc(){

        return zzjgmc;
   }

   /**
   * 字段名：zzjgmc
   * set方法
   * 备注: 
   */
   public void setZzjgmc(String zzjgmc){
        this.zzjgmc = zzjgmc;
   }
   /**
   * 字段名：jkkh
   * get方法
   * 备注: 
   */
   public String getJkkh(){

        return jkkh;
   }

   /**
   * 字段名：jkkh
   * set方法
   * 备注: 
   */
   public void setJkkh(String jkkh){
        this.jkkh = jkkh;
   }
   /**
   * 字段名：ksdm
   * get方法
   * 备注: 
   */
   public String getKsdm(){

        return ksdm;
   }

   /**
   * 字段名：ksdm
   * set方法
   * 备注: 
   */
   public void setKsdm(String ksdm){
        this.ksdm = ksdm;
   }
   /**
   * 字段名：ksmc
   * get方法
   * 备注: 
   */
   public String getKsmc(){

        return ksmc;
   }

   /**
   * 字段名：ksmc
   * set方法
   * 备注: 
   */
   public void setKsmc(String ksmc){
        this.ksmc = ksmc;
   }
   /**
   * 字段名：bqdm
   * get方法
   * 备注: 
   */
   public String getBqdm(){

        return bqdm;
   }

   /**
   * 字段名：bqdm
   * set方法
   * 备注: 
   */
   public void setBqdm(String bqdm){
        this.bqdm = bqdm;
   }
   /**
   * 字段名：bqmc
   * get方法
   * 备注: 
   */
   public String getBqmc(){

        return bqmc;
   }

   /**
   * 字段名：bqmc
   * set方法
   * 备注: 
   */
   public void setBqmc(String bqmc){
        this.bqmc = bqmc;
   }
   /**
   * 字段名：bfh
   * get方法
   * 备注: 
   */
   public String getBfh(){

        return bfh;
   }

   /**
   * 字段名：bfh
   * set方法
   * 备注: 
   */
   public void setBfh(String bfh){
        this.bfh = bfh;
   }
   /**
   * 字段名：bch
   * get方法
   * 备注: 
   */
   public String getBch(){

        return bch;
   }

   /**
   * 字段名：bch
   * set方法
   * 备注: 
   */
   public void setBch(String bch){
        this.bch = bch;
   }
   /**
   * 字段名：hzxm
   * get方法
   * 备注: 
   */
   public String getHzxm(){

        return hzxm;
   }

   /**
   * 字段名：hzxm
   * set方法
   * 备注: 
   */
   public void setHzxm(String hzxm){
        this.hzxm = hzxm;
   }
   /**
   * 字段名：sfzhm
   * get方法
   * 备注: 
   */
   public String getSfzhm(){

        return sfzhm;
   }

   /**
   * 字段名：sfzhm
   * set方法
   * 备注: 
   */
   public void setSfzhm(String sfzhm){
        this.sfzhm = sfzhm;
   }
   /**
   * 字段名：xbdm
   * get方法
   * 备注: 
   */
   public String getXbdm(){

        return xbdm;
   }

   /**
   * 字段名：xbdm
   * set方法
   * 备注: 
   */
   public void setXbdm(String xbdm){
        this.xbdm = xbdm;
   }
   /**
   * 字段名：xbmc
   * get方法
   * 备注: 
   */
   public String getXbmc(){

        return xbmc;
   }

   /**
   * 字段名：xbmc
   * set方法
   * 备注: 
   */
   public void setXbmc(String xbmc){
        this.xbmc = xbmc;
   }
   /**
   * 字段名：nls
   * get方法
   * 备注: 
   */
   public Short getNls(){

        return nls;
   }

   /**
   * 字段名：nls
   * set方法
   * 备注: 
   */
   public void setNls(Short nls){
        this.nls = nls;
   }
   /**
   * 字段名：nly
   * get方法
   * 备注: 
   */
   public String getNly(){

        return nly;
   }

   /**
   * 字段名：nly
   * set方法
   * 备注: 
   */
   public void setNly(String nly){
        this.nly = nly;
   }
   /**
   * 字段名：hyzkdm
   * get方法
   * 备注: 
   */
   public String getHyzkdm(){

        return hyzkdm;
   }

   /**
   * 字段名：hyzkdm
   * set方法
   * 备注: 
   */
   public void setHyzkdm(String hyzkdm){
        this.hyzkdm = hyzkdm;
   }
   /**
   * 字段名：hyzkmc
   * get方法
   * 备注: 
   */
   public String getHyzkmc(){

        return hyzkmc;
   }

   /**
   * 字段名：hyzkmc
   * set方法
   * 备注: 
   */
   public void setHyzkmc(String hyzkmc){
        this.hyzkmc = hyzkmc;
   }
   /**
   * 字段名：zylbdm
   * get方法
   * 备注: 
   */
   public String getZylbdm(){

        return zylbdm;
   }

   /**
   * 字段名：zylbdm
   * set方法
   * 备注: 
   */
   public void setZylbdm(String zylbdm){
        this.zylbdm = zylbdm;
   }
   /**
   * 字段名：zylbmc
   * get方法
   * 备注: 
   */
   public String getZylbmc(){

        return zylbmc;
   }

   /**
   * 字段名：zylbmc
   * set方法
   * 备注: 
   */
   public void setZylbmc(String zylbmc){
        this.zylbmc = zylbmc;
   }
   /**
   * 字段名：hzdhhm
   * get方法
   * 备注: 
   */
   public String getHzdhhm(){

        return hzdhhm;
   }

   /**
   * 字段名：hzdhhm
   * set方法
   * 备注: 
   */
   public void setHzdhhm(String hzdhhm){
        this.hzdhhm = hzdhhm;
   }
   /**
   * 字段名：zzlbdm
   * get方法
   * 备注: 
   */
   public String getZzlbdm(){

        return zzlbdm;
   }

   /**
   * 字段名：zzlbdm
   * set方法
   * 备注: 
   */
   public void setZzlbdm(String zzlbdm){
        this.zzlbdm = zzlbdm;
   }
   /**
   * 字段名：zzlbmc
   * get方法
   * 备注: 
   */
   public String getZzlbmc(){

        return zzlbmc;
   }

   /**
   * 字段名：zzlbmc
   * set方法
   * 备注: 
   */
   public void setZzlbmc(String zzlbmc){
        this.zzlbmc = zzlbmc;
   }
   /**
   * 字段名：dzsf
   * get方法
   * 备注: 
   */
   public String getDzsf(){

        return dzsf;
   }

   /**
   * 字段名：dzsf
   * set方法
   * 备注: 
   */
   public void setDzsf(String dzsf){
        this.dzsf = dzsf;
   }
   /**
   * 字段名：dzsq
   * get方法
   * 备注: 
   */
   public String getDzsq(){

        return dzsq;
   }

   /**
   * 字段名：dzsq
   * set方法
   * 备注: 
   */
   public void setDzsq(String dzsq){
        this.dzsq = dzsq;
   }
   /**
   * 字段名：dzx
   * get方法
   * 备注: 
   */
   public String getDzx(){

        return dzx;
   }

   /**
   * 字段名：dzx
   * set方法
   * 备注: 
   */
   public void setDzx(String dzx){
        this.dzx = dzx;
   }
   /**
   * 字段名：dzxz
   * get方法
   * 备注: 
   */
   public String getDzxz(){

        return dzxz;
   }

   /**
   * 字段名：dzxz
   * set方法
   * 备注: 
   */
   public void setDzxz(String dzxz){
        this.dzxz = dzxz;
   }
   /**
   * 字段名：dzc
   * get方法
   * 备注: 
   */
   public String getDzc(){

        return dzc;
   }

   /**
   * 字段名：dzc
   * set方法
   * 备注: 
   */
   public void setDzc(String dzc){
        this.dzc = dzc;
   }
   /**
   * 字段名：dzmphm
   * get方法
   * 备注: 
   */
   public String getDzmphm(){

        return dzmphm;
   }

   /**
   * 字段名：dzmphm
   * set方法
   * 备注: 
   */
   public void setDzmphm(String dzmphm){
        this.dzmphm = dzmphm;
   }
   /**
   * 字段名：yzbm
   * get方法
   * 备注: 
   */
   public String getYzbm(){

        return yzbm;
   }

   /**
   * 字段名：yzbm
   * set方法
   * 备注: 
   */
   public void setYzbm(String yzbm){
        this.yzbm = yzbm;
   }
   /**
   * 字段名：lxrxm
   * get方法
   * 备注: 
   */
   public String getLxrxm(){

        return lxrxm;
   }

   /**
   * 字段名：lxrxm
   * set方法
   * 备注: 
   */
   public void setLxrxm(String lxrxm){
        this.lxrxm = lxrxm;
   }
   /**
   * 字段名：lxrdhhm
   * get方法
   * 备注: 
   */
   public String getLxrdhhm(){

        return lxrdhhm;
   }

   /**
   * 字段名：lxrdhhm
   * set方法
   * 备注: 
   */
   public void setLxrdhhm(String lxrdhhm){
        this.lxrdhhm = lxrdhhm;
   }
   /**
   * 字段名：ryrq
   * get方法
   * 备注: 
   */
   public Date getRyrq(){

        return ryrq;
   }

   /**
   * 字段名：ryrq
   * set方法
   * 备注: 
   */
   public void setRyrq(Date ryrq){
        this.ryrq = ryrq;
   }
   /**
   * 字段名：cyrq
   * get方法
   * 备注: 
   */
   public Date getCyrq(){

        return cyrq;
   }

   /**
   * 字段名：cyrq
   * set方法
   * 备注: 
   */
   public void setCyrq(Date cyrq){
        this.cyrq = cyrq;
   }
   /**
   * 字段名：sjzyts
   * get方法
   * 备注: 
   */
   public Integer getSjzyts(){

        return sjzyts;
   }

   /**
   * 字段名：sjzyts
   * set方法
   * 备注: 
   */
   public void setSjzyts(Integer sjzyts){
        this.sjzyts = sjzyts;
   }
   /**
   * 字段名：ryqk
   * get方法
   * 备注: 
   */
   public String getRyqk(){

        return ryqk;
   }

   /**
   * 字段名：ryqk
   * set方法
   * 备注: 
   */
   public void setRyqk(String ryqk){
        this.ryqk = ryqk;
   }
   /**
   * 字段名：yxfzjcjg
   * get方法
   * 备注: 
   */
   public String getYxfzjcjg(){

        return yxfzjcjg;
   }

   /**
   * 字段名：yxfzjcjg
   * set方法
   * 备注: 
   */
   public void setYxfzjcjg(String yxfzjcjg){
        this.yxfzjcjg = yxfzjcjg;
   }
   /**
   * 字段名：zyszgcjg
   * get方法
   * 备注: 
   */
   public String getZyszgcjg(){

        return zyszgcjg;
   }

   /**
   * 字段名：zyszgcjg
   * set方法
   * 备注: 
   */
   public void setZyszgcjg(String zyszgcjg){
        this.zyszgcjg = zyszgcjg;
   }
   /**
   * 字段名：rzxyzdbm
   * get方法
   * 备注: 
   */
   public String getRzxyzdbm(){

        return rzxyzdbm;
   }

   /**
   * 字段名：rzxyzdbm
   * set方法
   * 备注: 
   */
   public void setRzxyzdbm(String rzxyzdbm){
        this.rzxyzdbm = rzxyzdbm;
   }
   /**
   * 字段名：rzxyzd
   * get方法
   * 备注: 
   */
   public String getRzxyzd(){

        return rzxyzd;
   }

   /**
   * 字段名：rzxyzd
   * set方法
   * 备注: 
   */
   public void setRzxyzd(String rzxyzd){
        this.rzxyzd = rzxyzd;
   }
   /**
   * 字段名：rzzybmdm
   * get方法
   * 备注: 
   */
   public String getRzzybmdm(){

        return rzzybmdm;
   }

   /**
   * 字段名：rzzybmdm
   * set方法
   * 备注: 
   */
   public void setRzzybmdm(String rzzybmdm){
        this.rzzybmdm = rzzybmdm;
   }
   /**
   * 字段名：rzzybm
   * get方法
   * 备注: 
   */
   public String getRzzybm(){

        return rzzybm;
   }

   /**
   * 字段名：rzzybm
   * set方法
   * 备注: 
   */
   public void setRzzybm(String rzzybm){
        this.rzzybm = rzzybm;
   }
   /**
   * 字段名：rzzyzhdm
   * get方法
   * 备注: 
   */
   public String getRzzyzhdm(){

        return rzzyzhdm;
   }

   /**
   * 字段名：rzzyzhdm
   * set方法
   * 备注: 
   */
   public void setRzzyzhdm(String rzzyzhdm){
        this.rzzyzhdm = rzzyzhdm;
   }
   /**
   * 字段名：rzzyzh
   * get方法
   * 备注: 
   */
   public String getRzzyzh(){

        return rzzyzh;
   }

   /**
   * 字段名：rzzyzh
   * set方法
   * 备注: 
   */
   public void setRzzyzh(String rzzyzh){
        this.rzzyzh = rzzyzh;
   }
   /**
   * 字段名：czxyzdbm
   * get方法
   * 备注: 
   */
   public String getCzxyzdbm(){

        return czxyzdbm;
   }

   /**
   * 字段名：czxyzdbm
   * set方法
   * 备注: 
   */
   public void setCzxyzdbm(String czxyzdbm){
        this.czxyzdbm = czxyzdbm;
   }
   /**
   * 字段名：czxyzd
   * get方法
   * 备注: 
   */
   public String getCzxyzd(){

        return czxyzd;
   }

   /**
   * 字段名：czxyzd
   * set方法
   * 备注: 
   */
   public void setCzxyzd(String czxyzd){
        this.czxyzd = czxyzd;
   }
   /**
   * 字段名：czzybmdm
   * get方法
   * 备注: 
   */
   public String getCzzybmdm(){

        return czzybmdm;
   }

   /**
   * 字段名：czzybmdm
   * set方法
   * 备注: 
   */
   public void setCzzybmdm(String czzybmdm){
        this.czzybmdm = czzybmdm;
   }
   /**
   * 字段名：czzybm
   * get方法
   * 备注: 
   */
   public String getCzzybm(){

        return czzybm;
   }

   /**
   * 字段名：czzybm
   * set方法
   * 备注: 
   */
   public void setCzzybm(String czzybm){
        this.czzybm = czzybm;
   }
   /**
   * 字段名：czzyzhdm
   * get方法
   * 备注: 
   */
   public String getCzzyzhdm(){

        return czzyzhdm;
   }

   /**
   * 字段名：czzyzhdm
   * set方法
   * 备注: 
   */
   public void setCzzyzhdm(String czzyzhdm){
        this.czzyzhdm = czzyzhdm;
   }
   /**
   * 字段名：czzyzh
   * get方法
   * 备注: 
   */
   public String getCzzyzh(){

        return czzyzh;
   }

   /**
   * 字段名：czzyzh
   * set方法
   * 备注: 
   */
   public void setCzzyzh(String czzyzh){
        this.czzyzh = czzyzh;
   }
   /**
   * 字段名：ssqklbdm
   * get方法
   * 备注: 
   */
   public String getSsqklbdm(){

        return ssqklbdm;
   }

   /**
   * 字段名：ssqklbdm
   * set方法
   * 备注: 
   */
   public void setSsqklbdm(String ssqklbdm){
        this.ssqklbdm = ssqklbdm;
   }
   /**
   * 字段名：ssqklbmc
   * get方法
   * 备注: 
   */
   public String getSsqklbmc(){

        return ssqklbmc;
   }

   /**
   * 字段名：ssqklbmc
   * set方法
   * 备注: 
   */
   public void setSsqklbmc(String ssqklbmc){
        this.ssqklbmc = ssqklbmc;
   }
   /**
   * 字段名：qkyhdjdm
   * get方法
   * 备注: 
   */
   public String getQkyhdjdm(){

        return qkyhdjdm;
   }

   /**
   * 字段名：qkyhdjdm
   * set方法
   * 备注: 
   */
   public void setQkyhdjdm(String qkyhdjdm){
        this.qkyhdjdm = qkyhdjdm;
   }
   /**
   * 字段名：qkyhdjmc
   * get方法
   * 备注: 
   */
   public String getQkyhdjmc(){

        return qkyhdjmc;
   }

   /**
   * 字段名：qkyhdjmc
   * set方法
   * 备注: 
   */
   public void setQkyhdjmc(String qkyhdjmc){
        this.qkyhdjmc = qkyhdjmc;
   }
   /**
   * 字段名：ssczbm
   * get方法
   * 备注: 
   */
   public String getSsczbm(){

        return ssczbm;
   }

   /**
   * 字段名：ssczbm
   * set方法
   * 备注: 
   */
   public void setSsczbm(String ssczbm){
        this.ssczbm = ssczbm;
   }
   /**
   * 字段名：ssjczmc
   * get方法
   * 备注: 
   */
   public String getSsjczmc(){

        return ssjczmc;
   }

   /**
   * 字段名：ssjczmc
   * set方法
   * 备注: 
   */
   public void setSsjczmc(String ssjczmc){
        this.ssjczmc = ssjczmc;
   }
   /**
   * 字段名：ssczksrq
   * get方法
   * 备注: 
   */
   public Date getSsczksrq(){

        return ssczksrq;
   }

   /**
   * 字段名：ssczksrq
   * set方法
   * 备注: 
   */
   public void setSsczksrq(Date ssczksrq){
        this.ssczksrq = ssczksrq;
   }
   /**
   * 字段名：mzffdm
   * get方法
   * 备注: 
   */
   public String getMzffdm(){

        return mzffdm;
   }

   /**
   * 字段名：mzffdm
   * set方法
   * 备注: 
   */
   public void setMzffdm(String mzffdm){
        this.mzffdm = mzffdm;
   }
   /**
   * 字段名：mzffmc
   * get方法
   * 备注: 
   */
   public String getMzffmc(){

        return mzffmc;
   }

   /**
   * 字段名：mzffmc
   * set方法
   * 备注: 
   */
   public void setMzffmc(String mzffmc){
        this.mzffmc = mzffmc;
   }
   /**
   * 字段名：ssgc
   * get方法
   * 备注: 
   */
   public String getSsgc(){

        return ssgc;
   }

   /**
   * 字段名：ssgc
   * set方法
   * 备注: 
   */
   public void setSsgc(String ssgc){
        this.ssgc = ssgc;
   }
   /**
   * 字段名：zfbm
   * get方法
   * 备注: 
   */
   public String getZfbm(){

        return zfbm;
   }

   /**
   * 字段名：zfbm
   * set方法
   * 备注: 
   */
   public void setZfbm(String zfbm){
        this.zfbm = zfbm;
   }
   /**
   * 字段名：zzzf
   * get方法
   * 备注: 
   */
   public String getZzzf(){

        return zzzf;
   }

   /**
   * 字段名：zzzf
   * set方法
   * 备注: 
   */
   public void setZzzf(String zzzf){
        this.zzzf = zzzf;
   }
   /**
   * 字段名：zyjzff
   * get方法
   * 备注: 
   */
   public String getZyjzff(){

        return zyjzff;
   }

   /**
   * 字段名：zyjzff
   * set方法
   * 备注: 
   */
   public void setZyjzff(String zyjzff){
        this.zyjzff = zyjzff;
   }
   /**
   * 字段名：zyyyff
   * get方法
   * 备注: 
   */
   public String getZyyyff(){

        return zyyyff;
   }

   /**
   * 字段名：zyyyff
   * set方法
   * 备注: 
   */
   public void setZyyyff(String zyyyff){
        this.zyyyff = zyyyff;
   }
   /**
   * 字段名：zlgcms
   * get方法
   * 备注: 
   */
   public String getZlgcms(){

        return zlgcms;
   }

   /**
   * 字段名：zlgcms
   * set方法
   * 备注: 
   */
   public void setZlgcms(String zlgcms){
        this.zlgcms = zlgcms;
   }
   /**
   * 字段名：cyqk
   * get方法
   * 备注: 
   */
   public String getCyqk(){

        return cyqk;
   }

   /**
   * 字段名：cyqk
   * set方法
   * 备注: 
   */
   public void setCyqk(String cyqk){
        this.cyqk = cyqk;
   }
   /**
   * 字段名：cyzztz
   * get方法
   * 备注: 
   */
   public String getCyzztz(){

        return cyzztz;
   }

   /**
   * 字段名：cyzztz
   * set方法
   * 备注: 
   */
   public void setCyzztz(String cyzztz){
        this.cyzztz = cyzztz;
   }
   /**
   * 字段名：cyyz
   * get方法
   * 备注: 
   */
   public String getCyyz(){

        return cyyz;
   }

   /**
   * 字段名：cyyz
   * set方法
   * 备注: 
   */
   public void setCyyz(String cyyz){
        this.cyyz = cyyz;
   }
   /**
   * 字段名：zljgdm
   * get方法
   * 备注: 
   */
   public String getZljgdm(){

        return zljgdm;
   }

   /**
   * 字段名：zljgdm
   * set方法
   * 备注: 
   */
   public void setZljgdm(String zljgdm){
        this.zljgdm = zljgdm;
   }
   /**
   * 字段名：zljgmc
   * get方法
   * 备注: 
   */
   public String getZljgmc(){

        return zljgmc;
   }

   /**
   * 字段名：zljgmc
   * set方法
   * 备注: 
   */
   public void setZljgmc(String zljgmc){
        this.zljgmc = zljgmc;
   }
   /**
   * 字段名：zyysbm
   * get方法
   * 备注: 
   */
   public String getZyysbm(){

        return zyysbm;
   }

   /**
   * 字段名：zyysbm
   * set方法
   * 备注: 
   */
   public void setZyysbm(String zyysbm){
        this.zyysbm = zyysbm;
   }
   /**
   * 字段名：zyysqm
   * get方法
   * 备注: 
   */
   public String getZyysqm(){

        return zyysqm;
   }

   /**
   * 字段名：zyysqm
   * set方法
   * 备注: 
   */
   public void setZyysqm(String zyysqm){
        this.zyysqm = zyysqm;
   }
   /**
   * 字段名：sjysbm
   * get方法
   * 备注: 
   */
   public String getSjysbm(){

        return sjysbm;
   }

   /**
   * 字段名：sjysbm
   * set方法
   * 备注: 
   */
   public void setSjysbm(String sjysbm){
        this.sjysbm = sjysbm;
   }
   /**
   * 字段名：sjysqm
   * get方法
   * 备注: 
   */
   public String getSjysqm(){

        return sjysqm;
   }

   /**
   * 字段名：sjysqm
   * set方法
   * 备注: 
   */
   public void setSjysqm(String sjysqm){
        this.sjysqm = sjysqm;
   }
   /**
   * 字段名：qmrq
   * get方法
   * 备注: 
   */
   public Date getQmrq(){

        return qmrq;
   }

   /**
   * 字段名：qmrq
   * set方法
   * 备注: 
   */
   public void setQmrq(Date qmrq){
        this.qmrq = qmrq;
   }
   /**
   * 字段名：gxsj
   * get方法
   * 备注: 
   */
   public Date getGxsj(){

        return gxsj;
   }

   /**
   * 字段名：gxsj
   * set方法
   * 备注: 
   */
   public void setGxsj(Date gxsj){
        this.gxsj = gxsj;
   }
   /**
   * 字段名：syxh
   * get方法
   * 备注: 
   */
   public String getSyxh(){

        return syxh;
   }

   /**
   * 字段名：syxh
   * set方法
   * 备注: 
   */
   public void setSyxh(String syxh){
        this.syxh = syxh;
   }
   /**
   * 字段名：tjzt
   * get方法
   * 备注: 
   */
   public String getTjzt(){

        return tjzt;
   }

   /**
   * 字段名：tjzt
   * set方法
   * 备注: 
   */
   public void setTjzt(String tjzt){
        this.tjzt = tjzt;
   }
   /**
   * 字段名：yxjl
   * get方法
   * 备注: 
   */
   public Integer getYxjl(){

        return yxjl;
   }

   /**
   * 字段名：yxjl
   * set方法
   * 备注: 
   */
   public void setYxjl(Integer yxjl){
        this.yxjl = yxjl;
   }

}